...
test.describe('Lexicon E2E New Project wizard app', () => {
  ...
  test.beforeAll(async ({ memberTab, request, manager, member }) => {
    ...
    existingProject.id = await initTestProject(
      request,
  ...
  test('Admin can get to wizard', async ({ adminTab }) => {
    const newLexProjectPageAdmin: NewLexProjectPage = new NewLexProjectPage(
      adminTab
    );
  ...
  test('Manager can get to wizard', async ({ managerTab }) => {
    const newLexProjectPageManager: NewLexProjectPage = new NewLexProjectPage(
      managerTab
    );
    await newLexProjectPageManager.goto();
    await expect(newLexProjectPageManager.newLexProjectForm).toBeVisible();
    await expect(
      newLexProjectPageManager.chooserPage.createButton
    ).toBeVisible();
  });
  ...
  test('Setup: user login and page contains a form', async () => {
    ...
    await expect(
      newLexProjectPageMember.chooserPage.createButton
    ).toBeVisible();
  });
  ...
  test.describe('Chooser page', () => {
    ...
    test('Can navigate to new project form and back', async () => {
      ...
      await expect(
        newLexProjectPageMember.namePage.projectNameInput
      ).toBeVisible();
      ...
      await expect(
        newLexProjectPageMember.chooserPage.sendReceiveButton
      ).toBeVisible();
    });
    ...
    test('Can navigate to Send and Receive form and back', async ({
      ...
      await expect(
        newLexProjectPageMember.srCredentialsPage.loginInput
      ).toBeVisible();
      await expect(
        newLexProjectPageMember.srCredentialsPage.loginInput
      ).toHaveValue(member.username);
      await expect(
        newLexProjectPageMember.srCredentialsPage.passwordInput
      ).toBeVisible();
      await expect(
        newLexProjectPageMember.srCredentialsPage.projectSelect
      ).not.toBeVisible();
  ...
  test.describe('Send Receive Credentials page', () => {
    ...
    test('Cannot move on if username is incorrect and can go back to Chooser page, user and password preserved', async ({
      ...
      await newLexProjectPageMember.srCredentialsPage.passwordInput.type(
        constants.passwordValid
      );
      // tab should trigger validation of the password thus making the test less flaky
      await newLexProjectPageMember.srCredentialsPage.passwordInput.press(
        'Tab'
      );
      await expect(
        newLexProjectPageMember.srCredentialsPage.credentialsInvalid
      ).toBeVisible();
      await newLexProjectPageMember.expectFormStatusHasNoError();
      await newLexProjectPageMember.nextButton.click();
      await expect(
      ...
      await expect(
        newLexProjectPageMember.srCredentialsPage.passwordInput
      ).toHaveValue(constants.passwordValid);
    });
    ...
    test('Cannot move on if credentials are invalid', async () => {
      await newLexProjectPageMember.srCredentialsPage.loginInput.fill(
        constants.srUsername
      );
      await newLexProjectPageMember.srCredentialsPage.passwordInput.fill(
        constants.passwordValid
      );
      await expect(
        newLexProjectPageMember.srCredentialsPage.credentialsInvalid
      ...
      await expect(
        newLexProjectPageMember.srCredentialsPage.loginOk
      ).not.toBeVisible();
      await newLexProjectPageMember.expectFormStatusHasNoError();
      await newLexProjectPageMember.nextButton.click();
      await expect(
    ...
    test('Can move on when the credentials are valid but cannot move further if no project is selected', async () => {
      ...
      await newLexProjectPageMember.srCredentialsPage.passwordInput.type(
        constants.srPassword
      );
      await expect(
        newLexProjectPageMember.srCredentialsPage.loginOk
      ).toBeVisible();
      await expect(
        newLexProjectPageMember.srCredentialsPage.passwordOk
      ).toBeVisible();
      await expect(
        newLexProjectPageMember.srCredentialsPage.loginInput
      ).toBeVisible();
    ...
    test('Cannot move on if not a manager of the project', async () => {
      ...
      await newLexProjectPageMember.srCredentialsPage.projectSelect.selectOption(
        { label: 'mock-name2 (mock-id2, contributor)' }
      );
      await expect(
        newLexProjectPageMember.srCredentialsPage.projectNoAccess
      ).toBeVisible();
      await newLexProjectPageMember.expectFormStatusHasError();
      await expect(newLexProjectPageMember.formStatus).toContainText(
    ...
    test('Can move on when a managed project is selected', async () => {
      ...
      await newLexProjectPageMember.srCredentialsPage.projectSelect.selectOption(
        { label: 'mock-name4 (mock-id4, manager)' }
      );
      await expect(
        newLexProjectPageMember.srCredentialsPage.projectOk
      ).toBeVisible();
      await newLexProjectPageMember.expectFormStatusHasNoError();
    });
  });
  ...
  test.describe('New Project', () => {
    ...
    test.describe('New Project Name page', () => {
      ...
      test('Finds the test project already exists', async () => {
        ...
        await expect(
          newLexProjectPageMember.namePage.projectCodeExists
        ).toBeVisible();
        await expect(
          newLexProjectPageMember.namePage.projectCodeAlphanumeric
        ).not.toBeVisible();
        await expect(
          newLexProjectPageMember.namePage.projectCodeOk
        ).not.toBeVisible();
        expect(
          await newLexProjectPageMember.namePage.projectCodeInput
        ).toHaveValue(existingProject.code);
        await newLexProjectPageMember.expectFormStatusHasError();
        await expect(newLexProjectPageMember.formStatus).toContainText(
      ...
      test.describe('Project Code tests', () => {
        ...
        test.describe('Edit Project Code', () => {
          test.beforeEach(async () => {
            await expect(
              newLexProjectPageMember.namePage.editProjectCodeCheckbox
            ).toBeVisible();
          ...
          test('Can edit project code when enabled', async () => {
            ...
            await newLexProjectPageMember.namePage.projectNameInput.press(
              'Tab'
            ); // trigger project code check
            await expect(
              newLexProjectPageMember.namePage.projectCodeInput
            ).toHaveValue('changed_new_project');
            await newLexProjectPageMember.expectFormStatusHasNoError();
          });

          test('Project code cannot be empty; does not show an error but is still invalid', async () => {
            await newLexProjectPageMember.namePage.projectCodeInput.fill('');
            await newLexProjectPageMember.namePage.projectCodeInput.press(
              'Tab'
            ); // trigger project code check
            await expect(
              newLexProjectPageMember.namePage.projectCodeExists
    ...
    test('Can create project, initial data page with upload & verify data', async () => {
      await newLexProjectPageMember.namePage.projectNameInput.type(
        newProject01.name
      );
      ...
      await expect(
        newLexProjectPageMember.initialDataPageBrowseButton
      ).toBeVisible();
      await newLexProjectPageMember.expectFormStatusHasNoError();
      ...
      await expect(
        newLexProjectPageMember.progressIndicatorStep3Label
      ).toHaveText('Verify');
      ...
      await expect(
        newLexProjectPageMember.verifyDataPage.entriesImported
      ).not.toBeVisible();
      ...
      expect(noticeElement.notice).toHaveCount(1);
      await expect(noticeElement.notice).toContainText(
        jpgFileName + ' is not an allowed compressed file. Ensure the file is'
      );
      await expect(
        newLexProjectPageMember.initialDataPageBrowseButton
      ).toBeVisible();
      ...
      await fileChooser3.setFiles(
        'test/e2e/shared-files/' + testLexProjectFileName
      );
      await expect(
        newLexProjectPageMember.verifyDataPage.entriesImported
      ).toBeVisible();
      expect(noticeElement.notice).toHaveCount(1);
      await expect(noticeElement.notice).toContainText(
        'Successfully imported ' + testLexProjectFileName
      );
      await newLexProjectPageMember.expectFormStatusHasNoError();
      ...
      // --displays stats ---------------------------------------------------------------
      await expect(newLexProjectPageMember.verifyDataPage.title).toHaveText(
        /Verify Data/
      );
      await expect(
        newLexProjectPageMember.verifyDataPage.entriesImported
      ).toHaveText(numberOfEntriesInTestLexProjectFile.toString());
      await newLexProjectPageMember.expectFormStatusHasNoError();
      ...
      await expect(
        newLexProjectPageMember.verifyDataPage.importErrors
      ).toHaveCount(1);
      await expect(
        newLexProjectPageMember.verifyDataPage.importErrors
      ).not.toBeVisible();
      ...
      await expect(
        newLexProjectPageMember.verifyDataPage.importErrors
      ).toContainText("range file 'TestProj.lift-ranges' was not found");
      await newLexProjectPageMember.verifyDataPage.nonCriticalErrorsButton.click();
      await expect(
        newLexProjectPageMember.verifyDataPage.importErrors
      ).not.toBeVisible();
      ...
      const entriesListPage: EntriesListPage = new EntriesListPage(
        newLexProjectPageMember.page,
        newProject01.id
      );
      await entriesListPage.expectTotalNumberOfEntries(
    ...
    test('Create: new empty project & can skip uploading data', async () => {
      ...
      await newLexProjectPageMember.nextButton.click();
      await expect(
        newLexProjectPageMember.primaryLanguagePageSelectButton
      ).toBeVisible();
    });
    ...
    test('Primary Language page', async () => {
      ...
      await expect(
        newLexProjectPageMember.selectLanguage.searchLanguageInput
      ).toBeVisible();
      await newLexProjectPageMember.selectLanguage.searchLanguageInput.fill(
        constants.searchLanguage
      );
      await newLexProjectPageMember.selectLanguage.searchLanguageInput.press(
        'Enter'
      );
      await expect(
        newLexProjectPageMember.selectLanguage.languageRows.first()
      ).toBeVisible();

      await expect(
        newLexProjectPageMember.selectLanguage.addButton
      ).toBeVisible();
      await expect(
        newLexProjectPageMember.selectLanguage.addButton
      ).not.toBeEnabled();
      ...
      await expect(newLexProjectPageMember.selectLanguage.addButton).toHaveText(
