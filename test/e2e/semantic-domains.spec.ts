...
test.describe('Lexicon E2E Semantic Domains Lazy Load', () => {
  ...
  const semanticDomain1dot1English =
    constants.testEntry1.senses[0].semanticDomain.values[0] + ' Sky';
  const semanticDomain1dot1Thai =
    constants.testEntry1.senses[0].semanticDomain.values[0] + ' ท้องฟ้า';
  ...
  test.beforeAll(async ({ request, managerTab, member, manager, admin }) => {
    ...
    const lexEntryId = await addLexEntry(
      request,
      project.code,
      constants.testEntry1
    );
  ...
  test('Should be using English Semantic Domain for manager', async () => {
    ...
    expect(
      editorPage.getTextarea(editorPage.entryCard, lexemeLabel, 'th')
    ).toHaveValue(constants.testEntry1.lexeme.th.value);
    await expect(
      editorPage.senseCard.locator(editorPage.semanticDomainSelector).first()
  ...
  test('Can change Project default language to Thai & back and forth', async () => {
    ...
    await expect(
      projectSettingsPage.projectTab.defaultInterfaceLanguageInput.locator(
        'option[selected="selected"]'
      )
    ).toHaveText('English');
    await projectSettingsPage.page.waitForTimeout(1000);
    await projectSettingsPage.projectTab.defaultInterfaceLanguageInput.selectOption(
      { label: 'ภาษาไทย - semantic domain only' }
    );
    await projectSettingsPage.page.waitForTimeout(2000);

    await projectSettingsPage.projectTab.saveButton.click();
    await expectOptionSelectedInSelectElement(
      projectSettingsPage.projectTab.defaultInterfaceLanguageInput,
      'ภาษาไทย'
    );
    ...
    ).toHaveText('ภาษาไทย - semantic domain only');
    await projectSettingsPage.page.waitForTimeout(1000);
    await projectSettingsPage.projectTab.defaultInterfaceLanguageInput.selectOption(
      { label: 'English' }
    );
    ...
    await pageHeader.languageDropdownItem
