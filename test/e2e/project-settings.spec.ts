...
test.describe('E2E Project Settings app', () => {
  ...
  test('Normal user cannot access projectSettings to a project of which the user is a member', async ({
    memberTab,
  }) => {
    const projectSettingsPage = new ProjectSettingsPage(memberTab);
    await projectSettingsPage.gotoProjectDirectly(
      projects[0].id,
      projects[0].name
    );
  ...
  test('Project owner can manage project they own', async ({ adminTab }) => {
    ...
    await projectSettingsPage.gotoProjectSettingsDirectly(
      projects[0].id,
      projects[0].name
    );
    ...
    await expect(
      projectSettingsPage.deleteTab.deleteProjectButton
    ).toBeVisible();
    await expect(
      projectSettingsPage.deleteTab.deleteProjectButton
    ).toBeDisabled();
  ...
  test('Manager cannot view delete tab if not owner', async ({ manager }) => {
    await projectSettingsPageManager.gotoProjectSettingsDirectly(
      projects[0].id,
      projects[0].name
    );
    expect(
      await projectSettingsPageManager.projectTab.projectOwner.innerText()
    ).not.toContain(manager.username);
    await expect(
      projectSettingsPageManager.deleteTab.tabTitle
    ).not.toBeVisible();
  });
  ...
  test('Manager can delete if owner', async () => {
    ...
    const nProjects =
      await projectSettingsPageManager.projectsPage.countProjects();
    await projectSettingsPageManager.gotoProjectSettingsDirectly(
      project4.id,
      project4.name
    ); // TODO: fix this flaky line
    expect(await projectSettingsPageManager.countNotices()).toBe(0);

    await projectSettingsPageManager.deleteTab.tabTitle.click();
    await expect(
      projectSettingsPageManager.deleteTab.deleteProjectButton
    ).toBeVisible();
    await expect(
      projectSettingsPageManager.deleteTab.deleteProjectButton
    ).toBeDisabled();
    await projectSettingsPageManager.deleteTab.confirmDeleteInput.fill(
      'delete'
    );
    await expect(
      projectSettingsPageManager.deleteTab.deleteProjectButton
    ...
    await projectSettingsPageManager.page.waitForNavigation({
      url: ProjectsPage.url,
    });
    //Or...
    // await projectSettingsPageManager.page.waitForNavigation({ waitUntil: 'networkidle' });

    expect(await projectSettingsPageManager.projectsPage.countProjects()).toBe(
      nProjects - 1
    );
  });
});
