...
test.describe('E2E Projects List app', () => {
  ...
  test.beforeAll(
    async ({ request, member, manager, memberTab, admin, adminTab }) => {
      ...
      project4.id = await initTestProject(
        request,
        project4.code,
        project4.name,
        manager.username,
      ...
      project5.id = await initTestProject(
        request,
        project5.code,
        project5.name,
        manager.username,
  ...
  test.describe('for Normal User', () => {
    ...
    test('Should list projects of which the user is a member', async () => {
      for (const project of projects) {
        expect(await projectsPageMember.findProject(project.name)).not.toMatch(
          '-1'
        );
      }
    });
  ...
  test.describe('for System Admin User', () => {
    ...
    test('Should list all projects', async () => {
      for (const project of [...projects, project4, project5]) {
        expect(await projectsPageAdmin.findProject(project.name)).not.toMatch(
          '-1'
        );
      }
      ...
      for (const project of [...projects, project5]) {
        await expect(
          await projectsPageAdmin.projectLinkLocator(project.name)
        ).not.toBeVisible();
      }
      await expect(
        await projectsPageAdmin.projectLinkLocator(project4.name)
      ).toBeVisible();
    });

    test('Should allow admin to add him- or herself to the project as tech support if not already a manager', async () => {
      expect(await projectsPageAdmin.projectIsLinked(project5.name)).toBe(
        false
      );
      expect(
        await projectsPageAdmin.projectHasAddTechSupportButton(project5.name)
      ).toBe(true);

      await (
        await projectsPageAdmin.projectAddTechSupportButtonLocator(
          project5.name
        )
      ).click();
      ...
      await expect(noticeElement.notice).toContainText(
        `You are now Tech Support for the '${project5.name}' project.`
      );
      await expect(
        await projectsPageAdmin.projectAddTechSupportButtonLocator(
          project5.name
      ...
      await expect(
        await projectsPageAdmin.projectLinkLocator(project5.name)
      ).toBeVisible();

      // admin is a contributor
      expect(await projectsPageAdmin.projectIsLinked(project4.name)).toBe(true);
      expect(
        await projectsPageAdmin.projectHasAddTechSupportButton(project4.name)
      ).toBe(true);
    });
  });
  ...
  test.describe('Lexicon E2E Project Access', () => {
    test('Admin added to project when accessing without membership', async () => {
      ...
      await expect(
        await projectsPageAdmin.projectLinkLocator(projects[2].name)
      ).not.toBeVisible();
      await gotoProjectDirectly(
        projectsPageAdmin.page,
        projects[2].id,
        projects[2].name
      );
      await projectsPageAdmin.goto();
    ...
    test('User redirected to projects app when accessing without membership', async ({
      ...
      await projectsPageMember.page.goto(
        '/app/lexicon/' + project5.id + '/#!/editor/list'
      );
      // redirect
      await expect(projectsPageMember.createButton).toBeVisible();
      expect(
        projectsPageMember.page.url().startsWith(baseURL + ProjectsPage.url)
      ).toBe(true);
    });
  });
});
