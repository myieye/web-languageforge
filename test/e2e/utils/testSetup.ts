...
export function initTestProject(
  ...
  return testControl(request, 'init_test_project', [
    projectCode,
    projectName,
    ownerUsername,
    memberUsernames,
...
export function addWritingSystemToProject(
  request: APIRequestContext,
  projectCode: string,
  languageTag: string,
  abbr = '',
  ...
  return testControl(request, 'add_writing_system_to_project', [
    projectCode,
    languageTag,
    abbr,
    name,
...
export function addUserToProject(
  ...
  return testControl(request, 'add_user_to_project', [
    projectCode,
    username,
    role,
  ]);
...
async function copyFileToSharedDir(filename: string): Promise<string> {
  ...
  if (commonDir) {
    ...
    throw new Error(
      'Dir test/common not found; E2E tests should be run from inside Git repo'
    );
  }
}
...
export async function addPictureFileToProject(
  request: APIRequestContext,
  projectCode: string,
  filename: string
) {
  const destPath = await copyFileToSharedDir(filename);
  return testControl(request, 'add_picture_file_to_project', [
    projectCode,
    destPath,
  ]);
}

export async function addAudioVisualFileToProject(
  request: APIRequestContext,
  projectCode: string,
  filename: string
) {
  const destPath = await copyFileToSharedDir(filename);
  return testControl(request, 'add_audio_visual_file_to_project', [
    projectCode,
    destPath,
  ]);
}
...
export function addCustomField(
  ...
  return testControl(request, 'add_custom_field', [
    projectCode,
    fieldName,
    parentField,
    fieldType,
...
export function addLexEntry(
  ...
  return testControl(request, 'add_lexical_entry', [
    projectCode,
    data,
  ]) as Promise<string>;
}
