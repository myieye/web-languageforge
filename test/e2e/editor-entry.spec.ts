...
import {
  addAudioVisualFileToProject,
  addLexEntry,
  addPictureFileToProject,
  addUserToProject,
  addWritingSystemToProject,
  initTestProject,
...
test.describe('Lexicon E2E Entry Editor and Entries List', () => {
  ...
  test.beforeAll(async ({ request, manager, managerTab, member, member2 }) => {
    project.id = await initTestProject(
      request,
      project.code,
      project.name,
      manager.username,
    ...
    await addWritingSystemToProject(
      request,
      project.code,
      'th-Zxxx-x-audio',
      'taud'
    ...
    await addPictureFileToProject(
      request,
      project.code,
      constants.testEntry1.senses[0].pictures[0].fileName
    );
    await addAudioVisualFileToProject(
      request,
      project.code,
      constants.testEntry1.lexeme['th-Zxxx-x-audio'].value
    );
    // put in data
    lexEntriesIds.push(
    ...
    lexEntriesIds.push(
      await addLexEntry(request, project.code, constants.testEntry2)
    );
    lexEntriesIds.push(
      await addLexEntry(
        request,
    ...
    editorPageManager = new EditorPage(
      managerTab,
      project.id,
      lexEntriesIds[0]
    );
  ...
  test.describe('Entries List', () => {
    ...
    test('Entries list has correct number of entries', async () => {
      await editorPageManager.entriesListPage.expectTotalNumberOfEntries(
        lexEntriesIds.length
      );
    });

    test('Search function works correctly', async () => {
      await editorPageManager.entriesListPage.filterInput.fill('asparagus');
      await expect(editorPageManager.entriesListPage.matchCount).toContainText(
        /1(?= \/)/
      );
    ...
    test('Can click on first entry', async () => {
      await editorPageManager.entriesListPage.clickOnEntry(
      ...
      await expect(
        editorPageManager.getTextarea(
          editorPageManager.entryCard,
          lexemeLabel,
          'th'
        )
  ...
  test.describe('Entry Editor', () => {
    ...
    test.skip('Can go from entry editor to entries list', async () => {
      ...
      expect(editorPageManager.page.url()).toContain(
        editorPageManager.entriesListPage.url
      );
    });
    ...
    test('Editor page has correct entry count in left side bar entries list', async () => {
      await editorPageManager.goto();
      await expect(editorPageManager.compactEntryListItem).toHaveCount(
        lexEntriesIds.length
      );
    });
    ...
    test('Add citation form as visible field', async () => {
      ...
      await (
        await editorPageManager.configurationPage.getCheckbox(
          'Entry Fields',
          'Citation Form',
          'Hidden if Empty'
        )
    ...
    test('Citation form field overrides lexeme form in dictionary citation view', async () => {
      ...
      await expect(editorPageManager.renderedDivs).not.toContainText([
        'citation form',
        'citation form',
      ]);
      if (
        (
          await editorPageManager.lexAppToolbar.toggleExtraFieldsButton.innerText()
        ).includes('Show Extra Fields')
      ) {
        await editorPageManager.lexAppToolbar.toggleExtraFieldsButton.click();
      }
      const citationFormInput = editorPageManager.getTextarea(
        editorPageManager.entryCard,
        'Citation Form',
        'th'
      );
    ...
    test.describe('Picture', () => {
      test('First picture and caption is present', async () => {
        ...
        const picture: Locator = await editorPageManager.getPicture(
          editorPageManager.senseCard,
          constants.testEntry1.senses[0].pictures[0].fileName
        );
        ...
        await expect(caption).toHaveValue(
          constants.testEntry1.senses[0].pictures[0].caption.en.value
        );
      });

      test('File upload drop box is displayed when Add Picture is clicked and can be cancelled', async () => {
        await editorPageManager.goto();
        const addPictureButton = editorPageManager.senseCard.locator(
          editorPageManager.addPictureButtonSelector
        );
        await expect(addPictureButton).toBeVisible();
        const dropbox = editorPageManager.senseCard.locator(
          editorPageManager.dropbox.dragoverFieldSelector
        );
        await expect(dropbox).not.toBeVisible();
        const cancelAddingPicture = editorPageManager.getCancelDropboxButton(
          editorPageManager.senseCard,
          'Picture'
        );
      ...
      test('Can change config to show pictures and hide empty captions & can change config to show empty captions', async () => {
        ...
        await (
          await editorPageManager.configurationPage.getFieldSpecificButton(
            'Meaning Fields',
            'Pictures'
          )
        ).click();
        await (
          await editorPageManager.configurationPage.getFieldSpecificCheckbox(
            'Meaning Fields',
            'Pictures',
      ...
      test('Picture is removed when Delete is clicked & can change config to hide pictures and hide captions', async () => {
        ...
        let picture: Locator = await editorPageManager.getPicture(
          editorPageManager.senseCard,
          constants.testEntry1.senses[0].pictures[0].fileName
        );
        expect(picture).not.toBeUndefined();
        await (
          await editorPageManager.getPictureDeleteButton(
            editorPageManager.senseCard,
            constants.testEntry1.senses[0].pictures[0].fileName
          )
        ).click();
        ...
        picture = await editorPageManager.getPicture(
          editorPageManager.senseCard,
          constants.testEntry1.senses[0].pictures[0].fileName
        );
        ...
        expect(picture).toBeUndefined();
        expect(
          editorPageManager.getPicturesOuterDiv(editorPageManager.senseCard)
        ).not.toBeVisible();
    ...
    test.describe('Audio', () => {
      ...
      test.describe('Member', () => {
        ...
        test.beforeAll(async ({ memberTab }) => {
          editorPageMember = new EditorPage(
            memberTab,
            project.id,
            lexEntriesIds[0]
          );
        ...
        test('Audio input system is present, playable and has "more" control (member)', async () => {
          ...
          const audio: Locator = editorPageMember.getSoundplayer(
            editorPageMember.entryCard,
            lexemeLabel,
            'taud'
          );
          await expect(audio).toBeVisible();
          await expect(
            audio.locator(editorPageMember.audioPlayer.playIconSelector)
          ).toBeVisible();
          // check if this audio player is the only one in this card
          await expect(
            editorPageMember.entryCard.locator(
              editorPageMember.audioPlayer.playIconSelector + ' >> visible=true'
            )
          ).toHaveCount(1);
          await expect(
            audio.locator(
              editorPageMember.audioPlayer.togglePlaybackAnchorSelector
            )
          ).toBeEnabled();

          await expect(
            audio.locator(editorPageMember.audioPlayer.dropdownToggleSelector)
          ).toBeVisible();
          await expect(
            audio.locator(editorPageMember.audioPlayer.dropdownToggleSelector)
          ).toBeEnabled();
          await expect(
            audio.locator(editorPageMember.audioPlayer.uploadButtonSelector)
          ).not.toBeVisible();
          // this button is only visible when user is observer and has only the right to download
          await expect(
            audio.locator(editorPageMember.audioPlayer.downloadButtonSelector)
          ).not.toBeVisible();
        });
      ...
      test.describe('Observer', () => {
        ...
        test.beforeAll(async ({ member2Tab }) => {
          editorPageObserver = new EditorPage(
            member2Tab,
            project.id,
            lexEntriesIds[0]
          );
        ...
        test('Audio Input System is playable but does not have "more" control (observer)', async () => {
          ...
          const audio: Locator = editorPageObserver.getSoundplayer(
            editorPageObserver.entryCard,
            lexemeLabel,
            'taud'
          );
          await expect(
            audio.locator(editorPageObserver.audioPlayer.playIconSelector)
          ).toBeVisible();
          // check if this audio player is the only one in this card
          await expect(
            editorPageObserver.entryCard.locator(
              editorPageObserver.audioPlayer.playIconSelector +
                ' >> visible=true'
            )
          ).toHaveCount(1);
          await expect(
            audio.locator(
              editorPageObserver.audioPlayer.togglePlaybackAnchorSelector
            )
          ).toBeVisible();
          await expect(
            audio.locator(
          ...
          await expect(
            audio.locator(editorPageObserver.audioPlayer.dropdownToggleSelector)
          ).not.toBeVisible();
          await expect(
            audio.locator(editorPageObserver.audioPlayer.uploadButtonSelector)
          ).not.toBeVisible();
          await expect(
            audio.locator(editorPageObserver.audioPlayer.downloadButtonSelector)
          ).toBeVisible();
        });
      ...
      test.describe('Manager', () => {
        let audio: Locator;
        test.beforeAll(async () => {
          audio = editorPageManager.getSoundplayer(
            editorPageManager.entryCard,
            lexemeLabel,
            'taud'
          );
        ...
        test('Audio input system is present, playable and has "more" control (manager)', async () => {
          ...
          await expect(audio).toBeVisible();
          await expect(
            audio.locator(editorPageManager.audioPlayer.playIconSelector)
          ).toBeVisible();
          // check if this audio player is the only one in this card
          await expect(
            editorPageManager.entryCard.locator(
              editorPageManager.audioPlayer.playIconSelector +
                ' >> visible=true'
            )
          ).toHaveCount(1);
          await expect(
            audio.locator(
              editorPageManager.audioPlayer.togglePlaybackAnchorSelector
            )
          ).toBeEnabled();
          await expect(
            audio.locator(editorPageManager.audioPlayer.dropdownToggleSelector)
          ).toBeVisible();
          await expect(
            audio.locator(editorPageManager.audioPlayer.dropdownToggleSelector)
          ).toBeEnabled();
          await expect(
            audio.locator(editorPageManager.audioPlayer.uploadButtonSelector)
          ).not.toBeVisible();
          // this button is only visible when user is observer and has only the right to download
          await expect(
            audio.locator(editorPageManager.audioPlayer.downloadButtonSelector)
          ).not.toBeVisible();
        });

        test('File upload drop box is displayed when Upload is clicked & not displayed if upload cancelled (manager)', async () => {
          await editorPageManager.goto();
          const dropbox = editorPageManager.entryCard.locator(
            editorPageManager.dropbox.dragoverFieldSelector
          );
          await expect(dropbox).not.toBeVisible();

          const cancelAddingAudio = editorPageManager.getCancelDropboxButton(
            editorPageManager.entryCard,
            'Audio'
          );
          ...
          await audio
            .locator(editorPageManager.audioPlayer.dropdownToggleSelector)
            .click();
          await editorPageManager.entryCard
            .locator(
              editorPageManager.audioDropdownMenu
                .uploadReplacementButtonSelector
            )
            .click();
          await expect(
            audio.locator(editorPageManager.audioPlayer.dropdownToggleSelector)
        ...
        test('Navigate to other entries with left entry bar', async () => {
          ...
          await editorPageManager.page
            .locator(
              'text=' +
                constants.testMultipleMeaningEntry1.senses[0].definition.en
                  .value
            )
            .click();
          ...
          await editorPageManager.page.waitForURL(
            new RegExp(
              `.*\/app\/lexicon\/${project.id}\/#!\/editor\/entry\/${lexEntriesIds[2]}.*`,
              'gm'
            )
          );
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard.first(),
              'Definition',
              'en'
            )
          ).toHaveValue(
            constants.testMultipleMeaningEntry1.senses[0].definition.en.value
          );
        });
        ...
        test('Can delete audio input system (manager)', async () => {
          ...
          await audio
            .locator(
              editorPageManager.audioDropdownMenu.deleteAudioButtonSelector
            )
            .click();
          const confirmModal = new ConfirmModalElement(editorPageManager.page);
          await confirmModal.confirmButton.click();
        ...
        test("Can't upload a non-audio file & can upload audio file", async () => {
          ...
          await audio
            .locator(editorPageManager.audioPlayer.uploadButtonSelector)
            .click();
          ...
          const [fileChooser] = await Promise.all([
            ...
            editorPageManager.page
              .locator(editorPageManager.dropbox.browseButtonSelector)
              .click(),
          ]);
          await fileChooser.setFiles(
            'test/e2e/shared-files/' + constants.testMockPngUploadFile.name
          );

          await expect(noticeElement.notice).toHaveCount(1);
          await expect(noticeElement.notice).toBeVisible();
          await expect(noticeElement.notice).toContainText(
            constants.testMockPngUploadFile.name +
          ...
          await fileChooser2.setFiles(
            'test/e2e/shared-files/' + constants.testMockMp3UploadFile.name
          );
          await expect(noticeElement.notice).toHaveCount(1);
          await expect(noticeElement.notice).toBeVisible();
          await expect(noticeElement.notice).toContainText(
        ...
        test('Dictionary citation reflects example sentences and translations', async () => {
          ...
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[0].examples[0].sentence
              .th.value,
            constants.testMultipleMeaningEntry1.senses[0].examples[0].sentence
              .th.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[0].examples[0]
              .translation.en.value,
            constants.testMultipleMeaningEntry1.senses[0].examples[0]
              .translation.en.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[0].examples[1].sentence
              .th.value,
            constants.testMultipleMeaningEntry1.senses[0].examples[1].sentence
              .th.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[0].examples[1]
              .translation.en.value,
            constants.testMultipleMeaningEntry1.senses[0].examples[1]
              .translation.en.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[1].examples[0].sentence
              .th.value,
            constants.testMultipleMeaningEntry1.senses[1].examples[0].sentence
              .th.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[1].examples[0]
              .translation.en.value,
            constants.testMultipleMeaningEntry1.senses[1].examples[0]
              .translation.en.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[1].examples[1].sentence
              .th.value,
            constants.testMultipleMeaningEntry1.senses[1].examples[1].sentence
              .th.value,
          ]);
          await expect(editorPageManager.renderedDivs).toContainText([
            constants.testMultipleMeaningEntry1.senses[1].examples[1]
              .translation.en.value,
            constants.testMultipleMeaningEntry1.senses[1].examples[1]
              .translation.en.value,
          ]);
        ...
        test('Word with multiple definitions: edit page has correct definitions, parts of speech', async () => {
          ...
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard.nth(1),
              'Definition',
              'en'
            )
          ).toHaveValue(
            constants.testMultipleMeaningEntry1.senses[1].definition.en.value
          );
        ...
        test('Word with multiple meanings: edit page has correct example sentences, translations', async () => {
          ...
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard
                .first()
                .locator(editorPageManager.exampleCardSelector + ' >> nth=0'),
              'Sentence',
              'th'
            )
          ).toHaveValue(
          ...
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard
                .first()
                .locator(editorPageManager.exampleCardSelector + ' >> nth=1'),
              'Sentence',
              'th'
            )
          ).toHaveValue(
        ...
        test('While Show Hidden Fields has not been clicked, hidden fields are hidden if they are empty', async () => {
          ...
          expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard.nth(0),
              'Semantics Note',
              'en'
            )
          ).toHaveCount(0);
        ...
        test('Word with multiple meanings: edit page has correct general notes, sources', async () => {
          ...
          await expect(
            ...
            constants.testMultipleMeaningEntry1.senses[0].generalNote.en.value
          );
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard.nth(1),
            ...
            constants.testMultipleMeaningEntry1.senses[1].generalNote.en.value
          );
          if (
            (
              await editorPageManager.lexAppToolbar.toggleExtraFieldsButton.innerText()
          ...
          await expect(
            ...
            constants.testMultipleMeaningEntry1.senses[0].source.en.value
          );
          await expect(
            editorPageManager.getTextarea(
              editorPageManager.senseCard.nth(1),
            ...
            constants.testMultipleMeaningEntry1.senses[1].source.en.value
          );
        });

        test('Senses can be reordered and deleted', async () => {
          await editorPageManager.goto(lexEntriesIds[2]);
          await editorPageManager.senseCard
            .first()
            .locator(editorPageManager.actionMenu.toggleMenuButtonSelector)
            .first()
            .click();
          await editorPageManager.senseCard
            .first()
            .locator(editorPageManager.actionMenu.moveDownButtonSelector)
            .first()
            .click();
          await expect(
            editorPageManager.getTextarea(
        ...
        test('Back to browse page, create new word, check word count, modify new word, autosaves changes, new word visible in editor and list', async () => {
          ...
          await editorPageManager
            .getTextarea(editorPageManager.entryCard, lexemeLabel, 'th')
            .fill(constants.testEntry3.lexeme.th.value);
          await editorPageManager
            .getTextarea(editorPageManager.senseCard, 'Definition', 'en')
          ...
          // Autosaves changes
          await editorPageManager.page.waitForURL(
            (url) => !url.hash.includes('editor/entry/_new_')
          );
          await editorPageManager.page.reload();
          ...
          await editorPageManager.search.searchInput.fill(
            constants.testEntry3.senses[0].definition.en.value
          );
          await expect(editorPageManager.search.matchCount).toContainText(
            /1(?= \/)/
          );

          // new word is visible in list page
          await editorPageManager.entriesListPage.goto();
          await editorPageManager.entriesListPage.filterInput.fill(
            constants.testEntry3.senses[0].definition.en.value
          );
          await expect(
            editorPageManager.entriesListPage.matchCount
          ).toContainText(/1(?= \/)/);
          await editorPageManager.entriesListPage.filterInputClearButton.click();
          ...
          await editorPageManager.entriesListPage.clickOnEntry(
            constants.testEntry3.lexeme.th.value
          );
          await editorPageManager.entryCard
            .first()
            .locator(editorPageManager.actionMenu.toggleMenuButtonSelector)
          ...
          await editorPageManager.entryCard
            .first()
            .locator(editorPageManager.actionMenu.deleteCardButtonSelector)
            .first()
            .click();
    ...
    test.describe('Configuration check', async () => {
      ...
      test('Can change configuration to make a writing system visible or invisible', async () => {
        ...
        expect(
          await editorPageManager.getNumberOfElementsWithSameLabel(
            editorPageManager.entryCard,
            lexemeLabel
          )
        ).toEqual(3);
        ...
        await expect(
          editorPageManager.getSoundplayer(
            editorPageManager.entryCard,
            lexemeLabel,
            'taud'
          )
      ...
      test('Make "taud" input system invisible for "Word" field and "tipa" invisible for manager role, then ensure it worked and change it back', async () => {
        ...
        await editorPageMember.goto();
        expect(
          await editorPageMember.getNumberOfElementsWithSameLabel(
            editorPageMember.entryCard,
            lexemeLabel
          )
        ).toEqual(2);
        await expect(
          editorPageMember.getTextarea(
            editorPageMember.entryCard,
            lexemeLabel,
            'th'
          )
