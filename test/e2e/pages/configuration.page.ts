...
export class ConfigurationPage {
  ...
  constructor(page: Page, projectId: string) {
    ...
    this.tabs = {
      fields: this.page.locator(
        '[data-ng-repeat="tab in $ctrl.tabs"] >> text=Fields >> visible=true'
      ),
    };
  ...
  async getRow(tableLocator: Locator, rowTitle: string): Promise<Locator> {
    const row: Locator = tableLocator.locator(
      `tr:has-text("${rowTitle}"):not(:has-text("Input Systems for")) >> visible=true`
    );
    ...
    if ((await row.count()) > 1)
      console.log(
        `Warning: more than 1 row was located with the row title ${rowTitle}. Proceeding with the first row.`
      );
    return row.first();
  }
  ...
  async getCheckbox(
    ...
    const columnIndex = await this.getChildPosition(
  ...
  async getFieldSpecificButton(
    tableTitle: string,
    rowTitle: string
  ): Promise<Locator> {
    const row: Locator = await this.getRow(
      await this.getTable(tableTitle),
      rowTitle
    );
    return row.locator('.field-specific-btn');
  }

  async getFieldSpecificCheckbox(
    tableTitle: string,
    rowTitle: string,
    ...
    const positionInTable = await this.getChildPosition(
      parentOfRow,
      'tr',
      await row.innerText()
    );
    let checkbox = parentOfRow
      .locator('tr')
      .nth(positionInTable + 2)
      .locator(
        `tr:has-text("${checkboxLabel}") >> input[type="checkbox"] >> visible=true`
      );
    if ((await checkbox.count()) == 0) {
      checkbox = parentOfRow
        .locator('tr')
        .nth(positionInTable + 1)
        .locator(`input[type="checkbox"] >> visible=true`);
    }
    return checkbox;
  }

  async getChildPosition(
    ...
    console.log(
      `Warning: there is no element with text ${innerText} with the parent ${parentLocator}.`
    );
    return undefined;
  }
}
