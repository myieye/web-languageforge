...
describe('Bellows E2E Projects List app', () => {
  ...
  describe('for Normal User', () => {
    it('should list the project of which the user is a member', async () => {
      ...
      expect(await projectsPage.projectNames.get(0).getText()).toBe(
    ...
    it('can list two projects of which the user is a member', async () => {
      ...
      await projectsPage.get();
      await browser.wait(
        ExpectedConditions.visibilityOf(projectsPage.createBtn),
        constants.conditionTimeout
      );
      await projectsPage.addMemberToProject(
  ...
  const shouldProjectBeLinked = async (
    projectName: string,
    projectRow: ElementFinder,
    bool: boolean
  ) => {
    expect<any>(
      await projectRow
        .element(by.cssContainingText('a', projectName))
        .isDisplayed()
    ).toBe(bool);
  };

  const shouldProjectHaveButtons = async (
    projectRow: ElementFinder,
    bool: boolean
  ) => {
  ...
  describe('for System Admin User', () => {
    it('should list all projects', async () => {
      ...
      return projectsPage
        .findProject(constants.testProjectName)
        .then((projectRow: ElementFinder) => {
          return shouldProjectBeLinked(
            constants.testProjectName,
            projectRow,
            true
          );
    ...
    it('should allow the admin to add themselves to the project as member or manager', async () => {
      ...
      await projectsPage
        ...
        .then(async (projectRow: ElementFinder) => {
          await shouldProjectBeLinked(
            constants.otherProjectName,
            projectRow,
            true
          );
