...
export class SfUserProfilePage {
  ...
  async getAboutMe() {
    ...
    return browser.wait(
      ExpectedConditions.visibilityOf(this.aboutMeTab.fullName),
  ...
  myAccountTab = {
    ...
    selectColor: (newColor: string | RegExp) => {
      return Utils.clickDropdownByValue(
        this.myAccountTab.avatarColor,
        newColor
      );
    },
    selectShape: (newShape: string | RegExp) => {
      return Utils.clickDropdownByValue(
        this.myAccountTab.avatarShape,
        newShape
      );
    },
    updateEmail: async (newEmail: string) => {
      await browser.wait(
        ExpectedConditions.visibilityOf(this.myAccountTab.emailInput),
        Utils.conditionTimeout
      );
      await this.myAccountTab.emailInput.sendKeys(
    ...
    updateUsername: async (newUsername: string) => {
      await browser.wait(
        ExpectedConditions.visibilityOf(this.myAccountTab.username),
        Utils.conditionTimeout
      );
      await this.myAccountTab.username.sendKeys(
        protractor.Key.chord(protractor.Key.CONTROL, 'a')
      );
    ...
    updateMobilePhone: async (newPhone: string) => {
      await browser.wait(
        ExpectedConditions.visibilityOf(this.myAccountTab.mobilePhoneInput),
        Utils.conditionTimeout
      );
  ...
  aboutMeTab = {
    ...
    updateFullName: async (newFullName: string) => {
      ...
      await this.aboutMeTab.fullName.sendKeys(
        protractor.Key.chord(protractor.Key.CONTROL, 'a')
      );
    ...
    updateAge: async (newAge: string) => {
      await browser.wait(
        ExpectedConditions.visibilityOf(this.aboutMeTab.age),
        Utils.conditionTimeout
      );
      await this.aboutMeTab.age.sendKeys(
        protractor.Key.chord(protractor.Key.CONTROL, 'a')
      );
