...
describe('Bellows E2E Reset Forgotten Password app', () => {
  ...
  it('with expired reset key routes to login with warning', async () => {
    ...
    await browser.wait(
      ExpectedConditions.visibilityOf(loginPage.errors.get(0)),
    ...
    expect<string>(await loginPage.errors.first().getText()).toContain(
      'expired'
    );
  ...
  describe('for Forgot Password request', () => {
    it('can navigate to request page', async () => {
      ...
      await browser.wait(
        ExpectedConditions.stalenessOf(loginPage.forgotPasswordLink),
        constants.conditionTimeout
      );
      await browser.wait(
        ExpectedConditions.visibilityOf(forgotPasswordPage.usernameInput),
        constants.conditionTimeout
      );
      expect<boolean>(
        await forgotPasswordPage.usernameInput.isDisplayed()
      ).toBe(true);
    });
    ...
    it('cannot request for non-existent user', async () => {
      ...
      await browser.wait(
        ExpectedConditions.visibilityOf(forgotPasswordPage.errors.get(0)),
        constants.conditionTimeout
      );
      expect<number>(await forgotPasswordPage.errors.count()).toBe(1);
      expect<string>(
        await forgotPasswordPage.errors.first().getText()
    ...
    it('can submit request', async () => {
      await forgotPasswordPage.usernameInput.sendKeys(
        constants.expiredUsername
      );
      await forgotPasswordPage.submitButton.click();
      expect<number>(await forgotPasswordPage.errors.count()).toBe(0);
      browser.wait(
        ExpectedConditions.stalenessOf(resetPasswordPage.confirmPasswordInput),
        constants.conditionTimeout
      );
      browser.wait(
        ExpectedConditions.visibilityOf(loginPage.infoMessages.get(0)),
        constants.conditionTimeout
      );
      expect<boolean>(await loginPage.username.isDisplayed()).toBe(true);
      expect<number>(await loginPage.errors.count()).toBe(0);
      expect<number>(await loginPage.infoMessages.count()).toBe(1);
      expect<string>(await loginPage.infoMessages.first().getText()).toContain(
        'email sent'
      );
    });
  });
  ...
  describe('for Reset Password', () => {
    it('with valid reset key routes reset page', async () => {
      ...
      await expect<boolean>(
        await resetPasswordPage.confirmPasswordInput.isDisplayed()
      ).toBe(true);
    ...
    it('refuses to allow form submission if the confirm input does not match', async () => {
      ...
      await resetPasswordPage.confirmPasswordInput.sendKeys(
        constants.passwordTooShort
      );
      expect<boolean>(await resetPasswordPage.resetButton.isEnabled()).toBe(
        false
      );
    ...
    it('should not allow a password less than 7 characters', async () => {
      await resetPasswordPage.passwordInput.sendKeys(
        constants.passwordTooShort
      );
      await resetPasswordPage.confirmPasswordInput.sendKeys(
        constants.passwordTooShort
