...
export class NewLexProjectPage {
  ...
  progressIndicatorStep3Label = element(
    by.id('progress-indicator-step3-label')
  );
  backButton = element(by.id('back-button'));
  nextButton = element(by.id('next-button'));

  async expectFormIsValid() {
    expect(await this.nextButton.getAttribute('class')).toMatch(
      /btn-primary(?:\s|$)/
    );
  }

  async expectFormIsNotValid() {
    expect(await this.nextButton.getAttribute('class')).not.toMatch(
      /btn-primary(?:\s|$)/
    );
  }
  ...
  formStatus = {
    async expectHasNoError() {
      expect(
        await element(by.id('form-status')).getAttribute('class')
    ...
    async expectContainsError(partialMsg: string) {
      ...
      expect(await element(by.id('form-status')).getText()).toContain(
        partialMsg
      );
    },
  };
