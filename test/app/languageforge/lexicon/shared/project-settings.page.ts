...
export class ProjectSettingsPage {
  ...
  static getTabByName(tabName: string) {
    return element(by.css('ul.nav-tabs')).element(
      by.cssContainingText('a', tabName)
    );
  }
  ...
  static getFieldByName(fieldName: string, treatAsRegex: boolean) {
    const fieldRegex: string | RegExp = treatAsRegex
      ? fieldName
      : new RegExp('^' + fieldName + '$');
    return element(By.css('div.tab-pane.active dl.picklists')).element(
      By.cssContainingText('div[data-ng-repeat]', fieldRegex)
    );
  }
}
