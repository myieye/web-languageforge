...
export class ConfigurationPage {
  ...
  unifiedPane = {
    inputSystem: {
      addGroupButton: this.activePane.element(
        by.id('add-group-input-system-btn')
      ),
      selectAll: {
        observer: this.activePane.element(
          by.id('input-system-select-all-observer-checkbox')
        ),
        commenter: this.activePane.element(
          by.id('input-system-select-all-commenter-checkbox')
        ),
        contributor: this.activePane.element(
          by.id('input-system-select-all-contributor-checkbox')
        ),
        manager: this.activePane.element(
          by.id('input-system-select-all-manager-checkbox')
        ),
        groups: () => {
          return this.activePane.all(
            by.className('input-system-select-all-group-checkbox')
          );
        },
      },
      rows: () => {
        return this.activePane.all(
          by.repeater(
            'inputSystem in $ctrl.unifiedViewModel.inputSystems.settings'
          )
        );
      },
      rowLabel: (rowIndex: number) => {
        return this.unifiedPane.inputSystem
          .rows()
          .get(rowIndex)
          .element(by.className('row-label'));
      },
      columnCheckboxes: (column: string) => {
        // column values: 'select-row', 'observer', 'commenter', 'contributor', 'manager'.
        return this.unifiedPane.inputSystem
          .rows()
          .all(by.className(column + '-checkbox'));
      },
      groupColumnCheckboxes: (groupIndex: number) => {
        return this.unifiedPane.inputSystem
          .rows()
          .all(by.className('checkbox-group-' + groupIndex));
      },
      removeGroupButton: (groupIndex: number) => {
        return this.activePane
          .element(by.id('table-header'))
          .element(by.className('remove-button-group-' + groupIndex));
      },
      addInputSystemButton: this.activePane.element(
        by.id('add-input-system-btn')
      ),
    },
    entry: {
      ...
      selectAll: {
        observer: this.activePane.element(
          by.id('entry-select-all-observer-checkbox')
        ),
        commenter: this.activePane.element(
          by.id('entry-select-all-commenter-checkbox')
        ),
        contributor: this.activePane.element(
          by.id('entry-select-all-contributor-checkbox')
        ),
        manager: this.activePane.element(
          by.id('entry-select-all-manager-checkbox')
        ),
        groups: () => {
          return this.activePane.all(
            by.className('entry-select-all-group-checkbox')
          );
        },
      },
      rows: () => {
        return this.activePane.all(
          by.repeater(
            'entryField in $ctrl.unifiedViewModel.entryFields.settings'
          )
        );
      },
      rowLabel: (rowIndex: number) => {
        return this.unifiedPane.entry
          .rows()
          .get(rowIndex)
          .element(by.className('row-label'));
      },
      ...
      removeGroupButton: (groupIndex: number) => {
        return this.activePane
          .element(by.id('entry-header'))
          .element(by.className('remove-button-group-' + groupIndex));
      },
      fieldSpecificInputSystemCheckbox: (
        label: string | RegExp,
        inputSystemSelector: string | number
      ) => {
        const candidates = this.getRowByLabel(label)
          .element(by.xpath('..'))
          .element(by.className('field-specific-input-systems'))
          .all(by.repeater('inputSystemSettings in entryField.inputSystems'));
        const filtered =
          typeof inputSystemSelector === 'number'
            ? candidates.get(inputSystemSelector)
            : candidates
                .filter((elem) =>
                  elem
                    .getText()
                    .then((text) => text.includes(inputSystemSelector))
                )
                .first();
    ...
    sense: {
      ...
      selectAll: {
        observer: this.activePane.element(
          by.id('sense-select-all-observer-checkbox')
        ),
        commenter: this.activePane.element(
          by.id('sense-select-all-commenter-checkbox')
        ),
        contributor: this.activePane.element(
          by.id('sense-select-all-contributor-checkbox')
        ),
        manager: this.activePane.element(
          by.id('sense-select-all-manager-checkbox')
        ),
        groups: () => {
          return this.activePane.all(
            by.className('sense-select-all-group-checkbox')
          );
        },
      },
      rows: () => {
        return this.activePane.all(
          by.repeater(
            'senseField in $ctrl.unifiedViewModel.senseFields.settings'
          )
        );
      },
      rowLabel: (rowIndex: number) => {
        return this.unifiedPane.sense
          .rows()
          .get(rowIndex)
          .element(by.className('row-label'));
      },
    ...
    example: {
      ...
      selectAll: {
        observer: this.activePane.element(
          by.id('example-select-all-observer-checkbox')
        ),
        commenter: this.activePane.element(
          by.id('example-select-all-commenter-checkbox')
        ),
        contributor: this.activePane.element(
          by.id('example-select-all-contributor-checkbox')
        ),
        manager: this.activePane.element(
          by.id('example-select-all-manager-checkbox')
        ),
        groups: () => {
          return this.activePane.all(
            by.className('example-select-all-group-checkbox')
          );
        },
      },
      rows: () => {
        return this.activePane.all(
          by.repeater(
            'exampleField in $ctrl.unifiedViewModel.exampleFields.settings'
          )
        );
      },
      rowLabel: (rowIndex: number) => {
        return this.unifiedPane.example
          .rows()
          .get(rowIndex)
          .element(by.className('row-label'));
      },
    ...
    hiddenIfEmptyCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('hidden-if-empty-checkbox')
      );
    },
    selectRowCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('select-row-checkbox')
      );
    },
    observerCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('observer-checkbox')
      );
    },
    commenterCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('commenter-checkbox')
      );
    },
    contributorCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('contributor-checkbox')
      );
    },
    managerCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('manager-checkbox')
      );
    },
    rowCheckboxes: (label: string | RegExp) => {
      return this.getRowByLabel(label).all(
        by.css('input[type="checkbox"]:not(.hidden-if-empty-checkbox)')
      );
    },
    fieldSpecificButton: (label: string | RegExp) => {
      return this.getRowByLabel(label).element(
        by.className('field-specific-btn')
      );
    },
    fieldSpecificIcon: (label: string | RegExp) => {
      return this.unifiedPane
        .fieldSpecificButton(label)
        .element(by.tagName('i'));
    },
    fieldSpecificCaptionHiddenIfEmptyCheckbox: (label: string | RegExp) => {
      return this.getRowByLabel(label)
        .all(by.xpath('following-sibling::tr'))
        .get(0)
        .element(by.className('caption-hidden-if-empty-checkbox'));
    },
    addGroupModal: {
      usernameTypeaheadInput: element(by.id('typeaheadInput')),
      usernameTypeaheadResults: element.all(
        by.repeater('user in $ctrl.typeahead.users')
      ),
      addMemberSpecificSettingsButton: element(
        by.id('add-member-specific-settings-btn')
      ),
    },
  };
  ...
  inputSystemsPane = {
    ...
    moreButtonGroup: {
      ...
      addVariant: this.activePane.element(
        by.id('configuration-add-variant-btn')
      ),
    ...
    getLanguageByName: (languageName: string) =>
      element(by.css('div.tab-pane.active div.col-md-3 dl.picklists')).element(
        by.cssContainingText('div[data-ng-repeat] span', languageName)
      ),

    selectedInputSystem: {
      displayName: this.activePane.element(by.id('languageDisplayName')),
      tag: this.activePane.element(
        by.binding(
          '$ctrl.iscInputSystemViewModels[$ctrl.selectedInputSystemId].inputSystem.tag'
        )
      ),
      abbreviationInput: this.activePane.element(
        by.model(
          '$ctrl.iscInputSystemViewModels[$ctrl.selectedInputSystemId].inputSystem.abbreviation'
        )
      ),
      rightToLeftCheckbox: this.activePane.element(
        by.model(
  ...
  private getRowByLabel(label: string | RegExp) {
    return this.activePane
      .element(by.cssContainingText('td', label))
      .element(by.xpath('..'));
  }
}
