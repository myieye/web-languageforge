...
export class EditorPage {
  ...
  firstNoticeCloseButton = this.noticeList
    .first()
  ...
  browse = {
    ...
    getEntryCount: async () => {
      ...
      await browser.wait(
        ExpectedConditions.visibilityOf(this.browse.entryCountElem),
        Utils.conditionTimeout
      );
      return this.browse.entryCountElem
        .getText()
        .then((s: string) => parseInt(/(\d+)$/.exec(s)[1], 10));
    },
    ...
    entriesList: this.browseDiv.all(
      by.repeater('entry in $ctrl.visibleEntries track by entry.id')
    ),
    clickEntryByLexeme: async (lexeme: string) => {
      await browser.wait(
        ExpectedConditions.visibilityOf(element(by.id('lexAppListView'))),
        Utils.conditionTimeout
      );
      const elements = await this.browse.entriesList.filter(
        async (row: ElementFinder) => {
          ...
          return elem
            .getText()
            .then((word: string) => word.indexOf(lexeme) > -1);
        }
      );
  ...
  edit = {
    ...
    toggleHiddenFieldsBtn: this.editToolbarDiv.element(
      by.id('toggleHiddenFieldsBtn')
    ),
    ...
    showHiddenFields: () => {
      ...
      return this.edit.toggleHiddenFieldsBtn
        .getText()
    ...
    getEntryCount: () => {
      return this.edit.entryCountElem
        .getText()
        .then((s: string) => parseInt(s, 10));
    },
    ...
    clickEntryByLexeme: (lexeme: string) => {
      ...
      return div.element(by.cssContainingText('.listItemPrimary', lexeme));
    },

    findEntryByDefinition: (definition: string) => {
      const div = this.editDiv.element(by.id('compactEntryListContainer'));
      return div.element(
        by.cssContainingText('.listItemSecondary', definition)
      );
    },
    ...
    search: {
      ...
      getMatchCount: () => {
        ...
        return this.edit.search.entryCountElem
          .getText()
          .then((s: string) => parseInt(/^(\d+)/.exec(s)[1], 10));
      },
    },
    ...
    // Top-row
    renderedDiv: this.appDiv.element(
      by.className('dc-rendered-entryContainer')
    ),
    actionMenu: this.editDiv.element(
      by.css('.entry-card .card-header .ellipsis-menu-toggle')
    ),
    deleteMenuItem: this.editDiv.element(
      by.css('.entry-card .card-header .dropdown-menu .dropdown-item')
    ),

    // Helper functions for retrieving various field values
    fields: this.editDiv.all(
      by.repeater('fieldName in $ctrl.config.fieldOrder')
    ),
    getLexemes: () => {
    ...
    getFirstLexeme: async () => {
      await browser.wait(
        ExpectedConditions.visibilityOf(this.edit.fields.get(0)),
        Utils.conditionTimeout
      );
    ...
    getLexemeByWsid: (searchWsid: string) => {
      ...
      return this.editorUtil
        .dcMultitextToObject(lexeme)
        .then((lexemes: string) => lexemes[searchWsid]);
    },
    ...
    audio: {
      ...
      moreControls: (searchLabel: string) => {
        return EditorUtil.getOneField(searchLabel).all(
          by.css('.dc-audio a.dropdown-toggle')
        );
      },

      moreGroups: (searchLabel: string, index: number) => {
        const allMoreGroups = EditorUtil.getOneField(searchLabel).all(
          by.css('.dc-audio .dropdown')
        );
        if (index !== undefined) {
          if (index < 0) index = 0;
      ...
      moreDownload: (searchLabel: string, index: number) => {
        return this.edit.audio
          .moreGroups(searchLabel, index)
          .element(by.className('dc-audio-download'));
      },

      moreDelete: (searchLabel: string, index: number) => {
        return this.edit.audio
          .moreGroups(searchLabel, index)
          .element(by.className('dc-audio-delete'));
      },

      moreUpload: (searchLabel: string, index: number) => {
        return this.edit.audio
          .moreGroups(searchLabel, index)
          .element(by.className('dc-audio-upload'));
      },

      uploadButtons: (searchLabel: string) => {
        return EditorUtil.getOneField(searchLabel).all(
          by.css('.dc-audio .upload-audio')
        );
      },
      ...
      uploadCancelButtons: (searchLabel: string) => {
        return EditorUtil.getOneField(searchLabel).all(
          by.css('.dc-audio i.fa-times')
        );
      },

      downloadButtons: (searchLabel: string) => {
        return EditorUtil.getOneField(searchLabel).all(
          by.css('.dc-audio a.buttonAppend')
        );
      },

      control: (searchLabel: string, index: number) => {
        const mockUploadElement = EditorUtil.getOneField(searchLabel)
          .all(by.css('.dc-audio'))
          .get(index);
    ...
    sense: {
      ...
      deleteSense: this.editDiv.all(
        by.css('dc-sense .ellipsis-menu-toggle ~ .dropdown-menu fa-trash')
      ),
      moveUp: this.editDiv.all(
        by.css('dc-sense .ellipsis-menu-toggle ~ .dropdown-menu .fa-arrow-up')
      ),
      moveDown: this.editDiv.all(
        by.css('dc-sense .ellipsis-menu-toggle ~ .dropdown-menu .fa-arrow-down')
      ),
    },
    ...
    pictures: {
      ...
      captions: EditorUtil.getOneField('Pictures').all(
        by.css('.input-group > .dc-text textarea')
      ),
      removeImages: EditorUtil.getOneField('Pictures').all(
        by.className('fa-trash')
      ),
      getFileName: (index: number) => {
        return this.editorUtil
          .getOneFieldValue('Pictures')
          .then((pictures: any) => pictures[index].fileName);
      },

      getCaption: (index: number) => {
        return this.editorUtil
          .getOneFieldValue('Pictures')
          .then((pictures: any) => pictures[index].caption);
      },

      addPictureLink: element(by.id('dc-picture-add-btn')),
      addDropBox: EditorUtil.getOneField('Pictures').element(
        by.css('.drop-box')
      ),
    ...
    semanticDomain: {
      values: EditorUtil.getOneField('Semantic Domain').all(
        by.className('dc-semanticdomain-value')
      ),
    },
  ...
  comment = {
    ...
    filter: {
      ...
      byStatusElem: this.commentDiv.element(
        by.model('$ctrl.commentFilter.status')
      ),
      clearElem: this.commentDiv.element(
        by.css('[title="Clear Filter] > i.fa-times')
      ),
      ...
      byStatus: (statusToFilterBy: string) => {
        Utils.clickDropdownByValue(
          this.comment.filter.byStatusElem,
          statusToFilterBy
        );
      },
    ...
    entry: {
      ...
      getFieldValues: this.editorUtil.getFieldValues,
      getOneFieldValue: this.editorUtil.getOneFieldValue,
      getOneFieldAllInputSystems: (
        searchLabel: string,
        idx: number = 0,
        rootElem: ElementFinder = element(by.className('dc-entry'))
      ) => {
        return EditorUtil.getOneField(searchLabel, idx, rootElem).all(
          by.css('span.wsid')
        );
      },
    },
    ...
    commentsList: this.commentDiv.all(
      by.repeater('comment in $ctrl.currentEntryCommentsFiltered')
    ),
  ...
  static getComment(commentsList: ElementArrayFinder, commentNum: number = 0) {
    const comment =
      commentNum === -1 ? commentsList.last() : commentsList.get(commentNum);
  ...
  static getReply(repliesList: ElementArrayFinder, replyNum: number) {
    ...
    const reply =
      replyNum === -1 ? repliesList.last() : repliesList.get(replyNum);
  ...
  static partsOfComment(div: ElementFinder) {
    ...
    return {
      ...
      author: div.element(
        by.binding('$ctrl.comment.authorInfo.createdByUserRef.name')
      ),
      date: div.element(
        by.binding('$ctrl.comment.authorInfo.createdDate | relativetime')
      ),
      ...
      plusOne: div.element(
        by.css('.comment-actions i.fa-thumbs-o-up:not(.ng-hide)')
      ),
      ...
      markOpenLink: div.element(
        by.css('.commentBottomBar i.fa-chevron-sign-up')
      ),
  ...
  static partsOfReply(div: ElementFinder) {
    return {
      ...
      date: div.element(
        by.model('reply.authorInfo.createdDate | relativetime')
      ),
