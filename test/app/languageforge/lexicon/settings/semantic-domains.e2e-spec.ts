...
describe('Lexicon E2E Semantic Domains Lazy Load', () => {
  ...
  it('should be using English Semantic Domain for manager', async () => {
    ...
    expect<any>(await editorPage.edit.getFirstLexeme()).toEqual(
      constants.testEntry1.lexeme.th.value
    );
    expect<any>(
      await editorPage.edit.semanticDomain.values.first().getText()
    ).toEqual(semanticDomain1dot1English);
  ...
  it('can change Project default language to Thai', async () => {
    ...
    expect<any>(await projectSettingsPage.tabs.project.isDisplayed()).toBe(
      true
    );
    expect<any>(
    ...
    expect<any>(
      await projectSettingsPage.projectTab.defaultLanguageSelect.isDisplayed()
    ).toBe(true);
    expect<any>(
      await projectSettingsPage.projectTab.defaultLanguageSelected.getText()
    ).toContain('English');
    await projectSettingsPage.projectTab.defaultLanguageSelect.sendKeys(
      'ภาษาไทย'
    );
  ...
  it('should be using Thai Semantic Domain', async () => {
    ...
    expect<any>(
      await editorPage.edit.semanticDomain.values.first().getText()
    ).toEqual(semanticDomain1dot1Thai);
  });
  ...
  it('can change Project default language back to Thai', async () => {
    ...
    await browser.wait(
      ExpectedConditions.visibilityOf(editorPage.edit.entryCountElem),
      Utils.conditionTimeout
    );
    await projectSettingsPage.getByLink();
    expect<any>(
