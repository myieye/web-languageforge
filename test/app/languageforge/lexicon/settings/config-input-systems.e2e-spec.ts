...
describe('Lexicon E2E Configuration Input Systems', () => {
  ...
  it('can select Input Systems tab', async () => {
    ...
    expect<any>(await configPage.inputSystemsPane.newButton.isDisplayed()).toBe(
      true
    );
    expect<any>(
      await configPage.inputSystemsPane.moreButton.isDisplayed()
    ).toBe(true);
  });

  it('can select an existing Input System', async () => {
    const language = 'Thai (IPA)';
    const inputSystem = await configPage.inputSystemsPane.getLanguageByName(
      language
    );
    expect<any>(inputSystem.isDisplayed()).toBe(true);
    inputSystem.click();
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.displayName.getText()
    ).toEqual(language);
    expect<any>(await configPage.applyButton.isEnabled()).toBe(false);
  });
  ...
  it('cannot change Special for an existing Input System', async () => {
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
    ).toEqual('th-fonipa');
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.specialDropdown.isDisplayed()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.specialDropdown.isEnabled()
    ).toBe(false);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.purposeDropdown.isDisplayed()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.purposeDropdown.isEnabled()
    ).toBe(false);
  });
  ...
  it('cannot add another IPA variation, but can add Voice and Variant', async () => {
    ...
    expect(
      await configPage.inputSystemsPane.moreButtonGroup.addIpa.getAttribute(
        'class'
      )
    ).toContain('disabled');
    expect(
      await configPage.inputSystemsPane.moreButtonGroup.addVoice.getAttribute(
        'class'
      )
    ).toContain('disabled');
    expect(
      await configPage.inputSystemsPane.moreButtonGroup.addVariant.getAttribute(
        'class'
      )
  ...
  it('cannot remove an existing Input System', async () => {
    expect<any>(
      await configPage.inputSystemsPane.moreButtonGroup.remove.isDisplayed()
    ).toBe(false);
  });

  describe('Select a new Input System Language modal', async () => {
    it('can open the new language modal', async () => {
      expect<any>(await configPage.inputSystemsPane.newButton.isEnabled()).toBe(
        true
      );
      await configPage.inputSystemsPane.newButton.click();
      expect<any>(
        await configPage.modal.selectLanguage.searchLanguageInput.isPresent()
      ).toBe(true);
    });

    it('can search for a language', async () => {
      expect<any>(
        await configPage.modal.selectLanguage.languageRows.count()
      ).toBe(0);
      await configPage.modal.selectLanguage.searchLanguageInput.sendKeys(
        firstLanguage + protractor.Key.ENTER
      );
      expect<any>(
        await configPage.modal.selectLanguage.languageRows.first().isPresent()
      ).toBe(true);
      expect<any>(
        await configPage.modal.selectLanguage.firstLanguageName.getText()
      ).toEqual(firstLanguage);
      expect<any>(
        await configPage.modal.selectLanguage.languageRows.last().isPresent()
      ).toBe(true);
      expect<any>(
        await configPage.modal.selectLanguage.lastLanguageName.getText()
      ).toEqual(lastLanguage);
    });

    it('can clear language search', async () => {
      expect<any>(
        await configPage.modal.selectLanguage.searchLanguageInput.getAttribute(
          'value'
        )
      ).toEqual(firstLanguage);
    ...
    it('can select language', async () => {
      ...
      expect<any>(
        await configPage.modal.selectLanguage.addButton.isPresent()
      ).toBe(true);
      expect<any>(
        await configPage.modal.selectLanguage.addButton.isEnabled()
      ).toBe(false);
      ...
      expect<any>(
        await configPage.modal.selectLanguage.addButton.getText()
      ).toEqual('Add ' + lastLanguage);
  ...
  it('can change Special to IPA', async () => {
    ...
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.ipaVariantInput.isDisplayed()
    ).toBe(false);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.voiceVariantInput.isDisplayed()
    ).toBe(false);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.scriptDropdown.isDisplayed()
    ).toBe(false);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.regionDropdown.isDisplayed()
    ).toBe(false);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.variantInput.isDisplayed()
    ).toBe(false);
    await Utils.clickDropdownByValue(
      configPage.inputSystemsPane.selectedInputSystem.specialDropdown,
      'IPA transcription'
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.purposeDropdown.isDisplayed()
  ...
  it('can change IPA Variant', async () => {
    await configPage.inputSystemsPane.selectedInputSystem.ipaVariantInput.sendKeys(
      'ngati' + protractor.Key.TAB
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can change IPA Purpose to Etic', async () => {
    await Utils.clickDropdownByValue(
      configPage.inputSystemsPane.selectedInputSystem.purposeDropdown,
      'Etic'
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can change Special to Voice', async () => {
    ...
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
    ).toEqual('mi-Zxxx-x-audio');
  });

  it('can change Voice Variant', async () => {
    await configPage.inputSystemsPane.selectedInputSystem.voiceVariantInput.sendKeys(
      'ngati' + protractor.Key.TAB
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can not add unspecified Variant', async () => {
    ...
    expect(await configPage.noticeList.get(0).getText()).toContain(
      'Specify at least one Script, Region or Variant'
    );
  ...
  it('can change Script', async () => {
    await Utils.clickDropdownByValue(
      configPage.inputSystemsPane.selectedInputSystem.scriptDropdown,
      new RegExp('Latin$')
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can change Region', async () => {
    await Utils.clickDropdownByValue(
      configPage.inputSystemsPane.selectedInputSystem.regionDropdown,
      'Cook Islands'
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can change Variant', async () => {
    await configPage.inputSystemsPane.selectedInputSystem.variantInput.sendKeys(
      'ngati' + protractor.Key.TAB
    );
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
  ...
  it('can add Variant variation', async () => {
    ...
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.scriptDropdown.isEnabled()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.regionDropdown.isDisplayed()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.regionDropdown.isEnabled()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.variantInput.isDisplayed()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.variantInput.isEnabled()
    ).toBe(true);
    expect<any>(
      await configPage.inputSystemsPane.selectedInputSystem.tag.getText()
    ).toEqual('mi-unspecified');
