...
describe('Lexicon E2E New Project wizard app', () => {
  ...
  describe('Chooser page', () => {
    ...
    it('can create a new project', async () => {
      ...
      expect<any>(await page.namePage.projectNameInput.isDisplayed()).toBe(
        true
      );
    });
    ...
    it('can go back to Chooser page', async () => {
      ...
      expect<any>(await page.chooserPage.sendReceiveButton.isDisplayed()).toBe(
        true
      );
    });

    it('can select Send and Receive', async () => {
      expect<any>(await page.chooserPage.sendReceiveButton.isEnabled()).toBe(
        true
      );
      await page.chooserPage.sendReceiveButton.click();
      expect<any>(await page.srCredentialsPage.loginInput.isDisplayed()).toBe(
        true
      );
    });
  ...
  describe('Send Receive Credentials page', () => {
    it('can get back to Send and Receive Credentials page', async () => {
      ...
      expect<any>(
        await page.srCredentialsPage.loginInput.getAttribute('value')
      ).toEqual(constants.memberUsername);
      expect<any>(
        await page.srCredentialsPage.passwordInput.isDisplayed()
      ).toBe(true);
      expect<any>(
        await page.srCredentialsPage.projectSelect().isPresent()
      ).toBe(false);
    });
    ...
    it('cannot move on if username is incorrect', async () => {
      ...
      await page.srCredentialsPage.passwordInput.sendKeys(
        constants.passwordValid
      );
      await browser.wait(
        ExpectedConditions.visibilityOf(
          page.srCredentialsPage.credentialsInvalid
        ),
        constants.conditionTimeout
      );
      expect<any>(
        await page.srCredentialsPage.credentialsInvalid.isDisplayed()
      ).toBe(true);
      await page.formStatus.expectHasNoError();
      await page.nextButton.click();
      expect<any>(await page.srCredentialsPage.loginInput.isDisplayed()).toBe(
      ...
      await page.formStatus.expectContainsError(
    ...
    it('can go back to Chooser page, user and pass preserved', async () => {
      ...
      expect(
        await page.srCredentialsPage.passwordInput.getAttribute('value')
    ...
    it('can move on when the credentials are valid', async () => {
      ...
      await browser.wait(
        ExpectedConditions.visibilityOf(page.srCredentialsPage.passwordOk),
        constants.conditionTimeout
      );
      expect<any>(await page.srCredentialsPage.loginOk.isDisplayed()).toBe(
        true
      );
      expect<any>(await page.srCredentialsPage.passwordOk.isDisplayed()).toBe(
        true
      );
      expect<any>(await page.srCredentialsPage.loginInput.isDisplayed()).toBe(
        true
      ...
      expect<any>(
        await page.srCredentialsPage.projectSelect().isDisplayed()
      ).toBe(true);
      await page.formStatus.expectHasNoError();
    });
    ...
    it('cannot move on if not a manager of the project', async () => {
      await Utils.clickDropdownByValue(
        page.srCredentialsPage.projectSelect(),
        'mock-name2'
      );
      expect<any>(
        await page.srCredentialsPage.projectNoAccess.isDisplayed()
      ).toBe(true);
      await page.formStatus.expectContainsError(
        'select a Project that you are the Manager of'
      );
    });
    ...
    it('can move on when a managed project is selected', async () => {
      ...
      expect<any>(await page.srCredentialsPage.projectOk.isDisplayed()).toBe(
        true
      );
      await page.formStatus.expectHasNoError();
  ...
  describe('Send Receive Verify page', () => {
    it('can clone project', async () => {
      await page.nextButton.click();
      await browser.wait(
        ExpectedConditions.visibilityOf(page.srClonePage.cloning),
        constants.conditionTimeout
      );
  ...
  describe('New Project Name page', () => {
    ...
    it('finds the test project already exists', async () => {
      await page.namePage.projectNameInput.sendKeys(
        constants.testProjectName + Key.TAB
      );
      await browser.wait(
        ExpectedConditions.visibilityOf(page.namePage.projectCodeExists),
        constants.conditionTimeout
      );
      expect<any>(await page.namePage.projectCodeExists.isDisplayed()).toBe(
        true
      );
      expect<any>(await page.namePage.projectCodeAlphanumeric.isPresent()).toBe(
        false
      );
      expect<any>(await page.namePage.projectCodeOk.isPresent()).toBe(false);
      expect(
        await page.namePage.projectCodeInput.getAttribute('value')
    ...
    it('with a cleared name does not show an error but is still invalid', async () => {
      ...
      await browser.wait(
        ExpectedConditions.stalenessOf(page.namePage.projectCodeExists),
        constants.conditionTimeout
      );
      expect<any>(await page.namePage.projectCodeExists.isPresent()).toBe(
        false
      );
      expect<any>(await page.namePage.projectCodeAlphanumeric.isPresent()).toBe(
        false
    ...
    it('can verify that an unused project name is available', async () => {
      ...
      await page.namePage.projectNameInput.sendKeys(
        constants.newProjectName + Key.TAB
      );
      await browser.wait(
        ExpectedConditions.visibilityOf(page.namePage.projectCodeOk),
        constants.conditionTimeout
      );
    ...
    it('can not edit project code by default', async () => {
      expect<any>(await page.namePage.projectCodeInput.isDisplayed()).toBe(
        false
      );
    });
    ...
    it('can edit project code when enabled', async () => {
      expect<any>(
        await page.namePage.editProjectCodeCheckbox.isDisplayed()
      ).toBe(true);
      await util.setCheckbox(page.namePage.editProjectCodeCheckbox, true);
      expect<any>(await page.namePage.projectCodeInput.isDisplayed()).toBe(
        true
      );
      await page.namePage.projectCodeInput.sendKeys(
        Key.chord(Key.CONTROL, 'a'),
        Key.BACK_SPACE
      );
    ...
    it('project code cannot be uppercase', async () => {
      ...
      await browser.wait(
        ExpectedConditions.visibilityOf(page.namePage.projectCodeAlphanumeric),
        constants.conditionTimeout
      );
      expect<any>(await page.namePage.projectCodeExists.isPresent()).toBe(
        false
      ...
      expect<any>(
        await page.namePage.projectCodeAlphanumeric.isDisplayed()
      ).toBe(true);
      expect<any>(await page.namePage.projectCodeOk.isPresent()).toBe(false);
      await page.formStatus.expectHasNoError();
      await page.nextButton.click();
    ...
    it('can create project', async () => {
      ...
      expect<any>(await page.initialDataPage.browseButton.isPresent()).toBe(
        true
      );
      await page.formStatus.expectHasNoError();
    });
  ...
  describe('Initial Data page with upload', () => {
    it('cannot see back button and defaults to uploading data', async () => {
      expect<any>(await page.backButton.isDisplayed()).toBe(false);
      expect<any>(await page.initialDataPage.browseButton.isDisplayed()).toBe(
        true
      );
      expect<any>(await page.progressIndicatorStep3Label.getText()).toEqual(
        'Verify'
      );
      await page.expectFormIsNotValid();
      await page.formStatus.expectHasNoError();
    ...
    describe('Mock file upload', () => {
      it('cannot upload large file', async () => {
        ...
        expect<any>(
          await page.initialDataPage.mockUpload.fileNameInput.isPresent()
        ).toBe(true);
        expect<any>(
          await page.initialDataPage.mockUpload.fileNameInput.isDisplayed()
        ).toBe(true);
        await page.initialDataPage.mockUpload.fileNameInput.sendKeys(
        ...
        expect<any>(await page.verifyDataPage.entriesImported.isPresent()).toBe(
          false
        );
        expect<any>(await page.noticeList.count()).toBe(1);
        expect<any>(await page.noticeList.get(0).getText()).toContain(
      ...
      it('cannot upload jpg', async () => {
        ...
        await page.initialDataPage.mockUpload.fileSizeInput.sendKeys(
        ...
        expect<any>(await page.noticeList.count()).toBe(1);
        expect(await page.noticeList.get(0).getText()).toContain(
          constants.testMockJpgImportFile.name +
      ...
      it('can upload zip file', async () => {
        ...
        await page.initialDataPage.mockUpload.uploadButton.click();
        expect<any>(
          await page.verifyDataPage.entriesImported.isDisplayed()
        ).toBe(true);
        expect<any>(await page.noticeList.count()).toBe(1);
        expect(await page.noticeList.get(0).getText()).toContain(
          'Successfully imported ' + constants.testMockZipImportFile.name
        );
        await page.formStatus.expectHasNoError();
      });
    });
  ...
  describe('Verify Data await page', () => {
    it('displays stats', async () => {
      expect<any>(await page.verifyDataPage.title.getText()).toEqual(
        'Verify Data'
      );
      expect<any>(await page.verifyDataPage.entriesImported.getText()).toEqual(
        '2'
      );
      await page.formStatus.expectHasNoError();
    });
    ...
    it('displays non-critical errors', async () => {
      expect<any>(await page.verifyDataPage.importErrors.isPresent()).toBe(
        true
      );
      expect<any>(await page.verifyDataPage.importErrors.isDisplayed()).toBe(
        false
      );
      ...
      expect(await page.verifyDataPage.importErrors.getText()).toContain(
        "range file 'TestProj.lift-ranges' was not found"
      );
      await page.verifyDataPage.nonCriticalErrorsButton.click();
      await browser.wait(
        ExpectedConditions.invisibilityOf(page.verifyDataPage.importErrors),
        constants.conditionTimeout
      );
      expect<any>(await page.verifyDataPage.importErrors.isDisplayed()).toBe(
        false
  ...
  describe('New Empty Project Name page', () => {
    it('create: new empty project', async () => {
      ...
      await page.chooserPage.createButton.click();
      await page.namePage.projectNameInput.sendKeys(
        constants.emptyProjectName + Key.TAB
      );
      await browser.wait(
        ExpectedConditions.visibilityOf(page.namePage.projectCodeOk),
        constants.conditionTimeout
  ...
  describe('Initial Data page skipping upload', () => {
    it('can skip uploading data', async () => {
      ...
      await page.expectFormIsNotValid();
      await page.nextButton.click();
      expect<any>(await page.primaryLanguagePage.selectButton.isPresent()).toBe(
        true
      );
    });
  });
  ...
  describe('Primary Language page', () => {
    ...
    it('can select language', async () => {
      expect<any>(await page.primaryLanguagePage.selectButton.isEnabled()).toBe(
        true
      );
      await page.primaryLanguagePage.selectButtonClick();
      expect<any>(
        await page.modal.selectLanguage.searchLanguageInput.isPresent()
      ).toBe(true);
    });

    describe('Select Language modal', () => {
      it('can search, select and add language', async () => {
        await page.modal.selectLanguage.searchLanguageInput.sendKeys(
          constants.searchLanguage + Key.ENTER
        );
        expect<any>(
          await page.modal.selectLanguage.languageRows.first().isPresent()
        ).toBe(true);

        expect<any>(await page.modal.selectLanguage.addButton.isPresent()).toBe(
          true
        );
        expect<any>(await page.modal.selectLanguage.addButton.isEnabled()).toBe(
          false
        );
        ...
        expect<any>(
          await page.modal.selectLanguage.addButton.getText()
        ).toEqual('Add ' + constants.foundLanguage);

        await page.modal.selectLanguage.addButton.click();
        await browser.wait(
          ExpectedConditions.stalenessOf(
            page.modal.selectLanguage.searchLanguageInput
          ),
          constants.conditionTimeout
        );
        expect<any>(
