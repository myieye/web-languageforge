...
export async function get({ project_code, cookie }) {
  const [{ id, projectName: name, users }, { entries, comments }] =
    await Promise.all([
  ...
  const unresolved_comments = comments.filter(
...
function has_audio(anEntry) {
  ...
  //		}
  // }
  const in_fields = (fields) =>
    Object.keys(fields).some((name) =>
      Object.keys(fields[name]).some(contains_audio)
    );
  ...
  // 		}]
  // }
  const in_example = (senses = []) =>
    senses.some((sense) => in_meaning(sense.examples));
