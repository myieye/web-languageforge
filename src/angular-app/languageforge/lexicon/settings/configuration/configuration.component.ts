...
export class LexiconConfigurationController implements angular.IController {
  ...
  lscOnUpdate: (params: {
    $event: { config?: LexiconConfig; optionLists?: LexOptionList[] };
  }) => void;
  ...
  constructor(
    private $scope: LexiconConfigControllerScope,
  ...
  $onChanges(changes: any): void {
    const configChange =
      changes.lscConfig as angular.IChangesObject<LexiconConfig>;
    ...
    const optionListsChange = changes.lscOptionLists as angular.IChangesObject<
      LexOptionList[]
    >;
  ...
  configurationApply(): void {
    ...
    this.lexProjectService.updateConfiguration(
      this.configDirty,
      this.optionListsDirty,
      (result) => {
        if (result.ok) {
          ...
          if (isSuccess) {
            ...
            this.sessionService.getSession().then((session) => {
              session.projectSettings<LexiconProjectSettings>().config =
                angular.copy(this.configPristine);
              session.projectSettings<LexiconProjectSettings>().optionlists =
                angular.copy(this.optionListsPristine);
            });
          } else {
    ...
    this.lscOnUpdate({
      $event: { config: this.configDirty, optionLists: this.optionListsDirty },
    });
  }
  ...
  onUpdate = ($event: {
    ...
    if ($event.pollUpdateTimerSecondsDirty > 0) {
      // Will be false if number is undefined
      this.configDirty.pollUpdateIntervalMs =
        $event.pollUpdateTimerSecondsDirty * 1000;
    }
  ...
  private setupView(): void {
    ...
    for (const inputSystemTag in this.configDirty.inputSystems) {
      if (this.configDirty.inputSystems.hasOwnProperty(inputSystemTag)) {
        ...
        const viewModel = new ConfigurationInputSystemsViewModel(
          this.selects,
          inputSystem
        );
      ...
      if (this.configDirty.entry.fields[fieldName] != null) {
        if (this.configDirty.entry.fields[fieldName].type !== 'fields') {
          this.fieldConfig[fieldName] =
            this.configDirty.entry.fields[fieldName];
        }
      }
    }

    const configSenses = this.configDirty.entry.fields
  ...
  private syncProjectStatusSuccess = (): void => {
    this.sessionService.getSession(true).then((session) => {
      this.configDirty = angular.copy(
        session.projectSettings<LexiconProjectSettings>().config
      );
      this.optionListsDirty = angular.copy(
        session.projectSettings<LexiconProjectSettings>().optionlists
      );
      this.lscOnUpdate({
        $event: {
          config: this.configDirty,
          optionLists: this.optionListsDirty,
        },
      });
      this.configPristine = angular.copy(this.configDirty);
      this.optionListsPristine = angular.copy(this.optionListsDirty);
    ...
    for (const inputSystemTag in this.inputSystemViewModels) {
      if (this.inputSystemViewModels.hasOwnProperty(inputSystemTag)) {
        ...
        if (viewModel.inputSystem.tag.includes('-unspecified')) {
          ...
          this.notice.push(
            this.notice.ERROR,
            'Specify at least one Script, Region or Variant for ' +
              viewModel.languageDisplayName()
          );
        }

        this.configDirty.inputSystems[viewModel.inputSystem.tag] =
          viewModel.inputSystem;
      }
    }
  ...
  private isPristine(): boolean {
    return (
      angular.equals(this.unifiedViewModelPristine, this.unifiedViewModel) &&
...
export const LexiconConfigurationComponent: angular.IComponentOptions = {
  ...
  templateUrl:
    '/angular-app/languageforge/lexicon/settings/configuration/configuration.component.html',
};
