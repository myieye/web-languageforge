...
export class ConfigurationFieldUnifiedViewModel {
  ...
  constructor(config: LexiconConfig, users: { [userId: string]: User }) {
    ...
    this.groupLists = ConfigurationFieldUnifiedViewModel.setGroupLists(
      config,
      users
    );
    this.inputSystems = new InputSystemSettingsList();
    const settings =
      ConfigurationFieldUnifiedViewModel.setInputSystemsViewModel(config);
    ...
    for (const tag in config.inputSystems) {
      if (config.inputSystems.hasOwnProperty(tag)) {
        const inputSystemViewModel = new ConfigurationInputSystemsViewModel(
          optionSelects,
          config.inputSystems[tag]
        );
        this.inputSystems.labels[tag] =
          inputSystemViewModel.languageDisplayName();
      }
    }

    const entryConfig = config.entry;
    this.entryFields = new FieldSettingsList();
    this.entryFields.settings =
      ConfigurationFieldUnifiedViewModel.setLevelViewModel(entryConfig, config);
    if ('senses' in entryConfig.fields) {
      ...
      this.senseFields.settings =
        ConfigurationFieldUnifiedViewModel.setLevelViewModel(
          sensesConfig,
          config
        );
      if ('examples' in sensesConfig.fields) {
        const examplesConfig = sensesConfig.fields
          .examples as LexConfigFieldList;
        this.exampleFields = new FieldSettingsList();
        this.exampleFields.settings =
          ConfigurationFieldUnifiedViewModel.setLevelViewModel(
            examplesConfig,
            config
          );
    ...
    for (const role of RoleType.roles()) {
      ConfigurationFieldUnifiedViewModel.checkIfAllRoleColumnSelected(
        this.inputSystems.settings,
        this.inputSystems.selectAllColumns,
        role
      );
      ConfigurationFieldUnifiedViewModel.checkIfAllRoleColumnSelected(
        this.entryFields.settings,
        this.entryFields.selectAllColumns,
        role
      );
      ConfigurationFieldUnifiedViewModel.checkIfAllRoleColumnSelected(
        this.senseFields.settings,
        this.senseFields.selectAllColumns,
        role
      );
      ConfigurationFieldUnifiedViewModel.checkIfAllRoleColumnSelected(
        this.exampleFields.settings,
        this.exampleFields.selectAllColumns,
        role
      );
    }
    ...
    for (let i = 0; i < this.inputSystems.settings[0].groups.length; i++) {
      ...
      ConfigurationFieldUnifiedViewModel.checkIfAllGroupColumnSelected(
        this.inputSystems.settings,
        this.inputSystems.selectAllColumns,
        i
      );
  ...
  toConfig(config: LexiconConfig): void {
    ...
    ConfigurationFieldUnifiedViewModel.inputSystemsToConfig(
      this.inputSystems,
      config,
      this.groupLists
    );
    ...
    const entryConfig = config.entry;
    ConfigurationFieldUnifiedViewModel.fieldsToConfig(
      this.entryFields.settings,
      this.inputSystems,
      config,
      entryConfig,
  ...
  static checkIfAllRoleColumnSelected(
    settings: SettingsBase[],
    selectAll: SettingsBase,
    role: string
  ): void {
  ...
  static checkIfAllGroupColumnSelected(
    settings: SettingsBase[],
    selectAll: SettingsBase,
    groupIndex: number
  ): void {
  ...
  static checkIfAllGroupSelected(
    setting: SettingsBase,
    settings: SettingsBase[],
    selectAll: SettingsBase,
    groupIndex: number
  ...
  private static inputSystemsToConfig(
    inputSystemSettings: InputSystemSettingsList,
    ...
    for (let i = 0; i < groupLists.length; i++) {
      const userView: LexUserViewConfig =
        config.userViews[groupLists[i].userId];
  ...
  private static fieldsToConfig(
    fields: FieldSettings[],
    inputSystemSettings: InputSystemSettingsList,
    config: LexiconConfig,
    levelConfig: LexConfigFieldList,
    groupLists: GroupList[]
  ): void {
    ...
    for (const field of fields) {
      ...
      if (levelConfigField.type === 'pictures') {
        (levelConfigField as LexConfigPictures).captionHideIfEmpty =
          field.captionHiddenIfEmpty;
      }
      // Create this array outside the if() because we'll use it in for loops lower down
      const fieldInputSystems = [];
      if (
        ConfigurationFieldUnifiedViewModel.isMultitextFieldType(
          levelConfigField.type
        )
      ) {
        const multiTextLevelConfigField =
          levelConfigField as LexConfigMultiText;
        multiTextLevelConfigField.inputSystems = [];
        for (const fieldInputSystemSettings of field.inputSystems) {
          if (
            fieldInputSystemSettings != null &&
            fieldInputSystemSettings.isAllRowSelected
          ) {
      ...
      for (const role of RoleType.roles()) {
        ...
        if (roleView != null && roleView.fields != null) {
          ...
          if (
            roleView.fields.hasOwnProperty(field.fieldName) &&
            ConfigurationFieldUnifiedViewModel.isMultitextFieldType(
              levelConfigField.type
            )
          ) {
            const roleFieldConfig = roleView.fields[
              field.fieldName
            ] as LexViewMultiTextFieldConfig;
            if (roleView.inputSystems && roleView.inputSystems.length) {
              const combinedTags = this.listIntersection(
                fieldInputSystems,
                roleView.inputSystems
              );
      ...
      for (let i = 0; i < groupLists.length; i++) {
        ...
        if (
          userView.fields.hasOwnProperty(field.fieldName) &&
          ConfigurationFieldUnifiedViewModel.isMultitextFieldType(
            levelConfigField.type
          )
        ) {
          const userFieldConfig = userView.fields[
            field.fieldName
          ] as LexViewMultiTextFieldConfig;
  ...
  private static setInputSystemsViewModel(
    config: LexiconConfig
  ): InputSystemSettings[] {
    const inputSystems: InputSystemSettings[] = [];
    const selectedManagerTags =
      ConfigurationFieldUnifiedViewModel.getSelectedInputSystemsManagerTags(
        config
      );
    let i = 0;
    for (const tag of selectedManagerTags) {
      ConfigurationFieldUnifiedViewModel.setInputSystemViewModel(
        config,
        inputSystems,
        tag,
        i
    ...
    for (const tag in config.inputSystems) {
      if (
        config.inputSystems.hasOwnProperty(tag) &&
        !selectedManagerTags.includes(tag)
      ) {
        ConfigurationFieldUnifiedViewModel.setInputSystemViewModel(
          config,
          inputSystems,
  ...
  private static setInputSystemViewModel(
    config: LexiconConfig,
    ...
    for (const userId in config.userViews) {
      if (
        config.userViews.hasOwnProperty(userId) &&
        config.userViews[userId] != null &&
  ...
  private static addWithoutDuplicates(
    orig: string[],
    newItems: string[]
  ): void {
  ...
  private static getSelectedInputSystemsManagerTags(
    config: LexiconConfig
  ): string[] {
    const roleType = new RoleType();
    ...
    if (roleView != null && roleView.fields != null) {
      for (const fieldName in roleView.fields) {
        if (
          roleView.fields.hasOwnProperty(fieldName) &&
          ConfigurationFieldUnifiedViewModel.isMultitextFieldType(
            roleView.fields[fieldName].type
          )
        ) {
          const multiTextField = roleView.fields[
            fieldName
          ] as LexViewMultiTextFieldConfig;
          if (multiTextField.overrideInputSystems) {
            ConfigurationFieldUnifiedViewModel.addWithoutDuplicates(
              tags,
              multiTextField.inputSystems
            );
          }
        }
      }
  ...
  private static setInputSystemRoleSettings(
    tag: string,
    config: LexiconConfig,
    role: string,
    roleType: string,
  ...
  private static setInputSystemGroupSettings(
    tag: string,
    config: LexiconConfig,
    userId: string,
    groupIndex: number,
    ...
    if (userView != null) {
      if (userView.inputSystems && userView.inputSystems.length) {
        inputSystemSettings.groups[groupIndex].show =
          userView.inputSystems.includes(tag);
      } else {
  ...
  private static setLevelViewModel(
    levelConfig: LexConfigFieldList,
    config: LexiconConfig
  ): FieldSettings[] {
    ...
    for (const fieldName of levelConfig.fieldOrder) {
      if (
        fieldName in levelConfig.fields &&
        levelConfig.fields[fieldName].type !== 'fields'
      ) {
        ...
        if (levelConfig.fields[fieldName].type === 'pictures') {
          fieldSettings.captionHiddenIfEmpty = (
            levelConfig.fields[fieldName] as LexConfigPictures
          ).captionHideIfEmpty;
        }
        ConfigurationFieldUnifiedViewModel.setLevelRoleSettings(
          fieldName,
          config,
          fieldSettings
        );
        ConfigurationFieldUnifiedViewModel.setLevelGroupSettings(
          fieldName,
          config,
          fieldSettings
        );
        ...
        if (
          ConfigurationFieldUnifiedViewModel.isMultitextFieldType(
            levelConfig.fields[fieldName].type
          )
        ) {
          const multiTextLevelConfigField = levelConfig.fields[
            fieldName
          ] as LexConfigMultiText;
          ...
          for (const tag in config.inputSystems) {
            if (
              config.inputSystems.hasOwnProperty(tag) &&
              !multiTextLevelConfigField.inputSystems.includes(tag)
            ) {
              const inputSystemSettings = new InputSystemSettings();
              inputSystemSettings.tag = tag;
              inputSystemSettings.fieldName = fieldName;
  ...
  private static setLevelRoleSettings(
    fieldName: string,
    config: LexiconConfig,
  ...
  private static setLevelGroupSettings(
    fieldName: string,
    config: LexiconConfig,
    fieldSettings: FieldSettings
  ): void {
    ...
    for (const userId in config.userViews) {
      if (
        ...
        fieldSettings.groups[groupIndex++].show =
          config.userViews[userId].fields[fieldName].show;
      }
    }
  }

  private static setGroupLists(
    config: LexiconConfig,
