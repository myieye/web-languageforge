...
export class LexiconConfigService {
  ...
  getEditorConfig(
    updatedConfig?: LexiconConfig,
    updatedOptionLists?: LexOptionList[]
    ...
    return this.sessionService.getSession().then((session) => {
      ...
      const role =
        session.projectSettings<LexiconProjectSettings>().currentUserRole;
      let fieldsConfig;

      // copy option lists to config object
      config.optionlists = {};
      for (const optionList of session.projectSettings<LexiconProjectSettings>()
        .optionlists) {
      ...
      if (fieldsConfig != null) {
        ...
        this.removeDisabledConfigFields(
          config.entry.fields.senses as LexConfigFieldList,
          fieldsConfig
        );
        this.removeDisabledConfigFields(
          (config.entry.fields.senses as LexConfigFieldList).fields
  ...
  private removeDisabledConfigFields(
    ...
    const visibleFields = config.fieldOrder.filter((fieldName: string) => {
      ...
      if (fieldConfig && fieldConfig.show) {
        ...
        if (fieldConfig.overrideInputSystems) {
          (config.fields[fieldName] as LexConfigMultiText).inputSystems =
            angular.copy(fieldConfig.inputSystems);
        }
