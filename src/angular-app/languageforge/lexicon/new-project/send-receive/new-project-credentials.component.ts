...
import {
  LexiconProject,
  SendReceiveProject,
} from '../../shared/model/lexicon-project.model';
...
export class NewProjectSendReceiveCredentialsController
  implements angular.IController
{
  ...
  checkSRProject() {
    ...
    this.sendReceiveApi
      .getUserProjects(
        this.srcProject.sendReceive.username,
        this.srcProject.sendReceive.password
      )
      .then((result) => {
  ...
  showProjectSelect() {
    ...
    if (
      show &&
      this.srcProject.sendReceive.project != null &&
      this.srcProject.sendReceive.project.identifier != null &&
      this.srcProject.sendReceive.projects != null
    ) {
      for (
        let index = 0;
        index < this.srcProject.sendReceive.projects.length;
        index++
      ) {
        const project = this.srcProject.sendReceive.projects[index];
        if (
          project.identifier ===
            this.srcProject.sendReceive.project.identifier &&
...
export const NewProjectSendReceiveCredentialsState = {
  ...
  data: {
    ...
    isFormValid(
      ...
      if (
        controller.project.sendReceive.project != null &&
        controller.project.sendReceive.project.isLinked
      ) {
      ...
      if (!controller.project.sendReceive.username) {
        return controller.error(
          'Login cannot be empty. Please enter your LanguageDepot.org login username.'
        );
      }

      if (!controller.project.sendReceive.password) {
        return controller.error(
          'Password cannot be empty. Please enter your LanguageDepot.org password.'
        );
      }
      ...
      if (
        ...
        return controller.error(
          'Please select a Project that you are the Manager of on LanguageDepot.org.'
        );
      }
    ...
    goNextState(controller: LexiconNewProjectController): void {
      ...
      if (controller.project.sendReceive.project.isLinked) {
        ...
        controller.projectService
          .joinSwitchSession(
            controller.project.sendReceive.project.identifier,
            role
          )
          .then((result) => {
            if (result.ok) {
              ...
              controller.sessionService
                .getSession(true)
                .then(controller.gotoEditor);
            } else {
              controller.notice.push(
                controller.notice.ERROR,
        ...
        controller.newProject.projectName =
          controller.project.sendReceive.project.name;
        controller.newProject.projectCode =
          controller.project.sendReceive.project.identifier;
        controller.projectService
          .projectCodeExists(controller.newProject.projectCode)
          .then((result) => {
