...
export class LexiconNewProjectController implements angular.IController {
  ...
  $onInit(): void {
    ...
    this.applicationHeaderService.setPageName(
      'Start or join a Web Dictionary Project'
    );
  ...
  checkProjectCode(): angular.IPromise<string> {
    this.projectCodeStateDefer = this.$q.defer();
    if (
      !LexiconProjectService.isValidProjectCode(this.newProject.projectCode)
    ) {
      ...
      this.projectService.projectCodeExists(
  ...
  createProject(): angular.IPromise<void> {
    ...
    return this.projectService
      .createSwitchSession(
        ...
        this.newProject.projectCode,
        this.newProject.appName,
        this.project.sendReceive.project
      )
      .then((result) => {
        if (result.ok) {
  ...
  getProject = (): void => {
    this.sendReceiveApi.receiveProject((result) => {
      if (result.ok) {
        ...
        this.notice.push(
          this.notice.ERROR,
          'The project could not be synchronized with LanguageDepot.org. ' +
            'Please try again.'
        );
        this.gotoEditor();
      }
  ...
  savePrimaryLanguage(callback?: () => void): void {
    ...
    this.sessionService.getSession().then((session) => {
      ...
      if (
        this.newProject.languageCode !== 'th' &&
        'th' in config.inputSystems
      ) {
        delete config.inputSystems.th;
        this.replaceFieldInputSystem(
          config.entry,
          'th',
          this.newProject.languageCode
        );
      }
        ...
        (result) => {
          ...
          if (result.ok) {
            ...
            this.makeFormInvalid(
              'Could not add ' + this.newProject.language.name + ' to project.'
            );
          }
        }
      );
  ...
  private replaceFieldInputSystem(
...
export const LexiconNewProjectComponent: angular.IComponentOptions = {
  ...
  templateUrl:
    '/angular-app/languageforge/lexicon/new-project/lexicon-new-project.component.html',
};
