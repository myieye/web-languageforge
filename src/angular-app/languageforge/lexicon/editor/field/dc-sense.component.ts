...
export class FieldSenseController implements angular.IController {
  ...
  deleteExample = (index: number): void => {
    const deletemsg =
      "Are you sure you want to delete the example <b>' " +
      LexiconUtilityService.getExample(
        this.control.config,
        this.config.fields.examples as LexConfigFieldList,
        this.model.examples[index],
        'sentence'
      ) +
      " '</b>";
    this.modal
      .showModalSimple('Delete Example', deletemsg, 'Cancel', 'Delete Example')
      .then(
        () => {
          // Adding or removing examples makes for a non-delta update, so save a possible delta update first
          this.control.saveCurrentEntry(false, () => {
...
export const FieldSenseComponent: angular.IComponentOptions = {
  ...
  templateUrl:
    '/angular-app/languageforge/lexicon/editor/field/dc-sense.component.html',
};
