...
export class CommentController implements angular.IController {
  ...
  constructor(
    ...
    private configService: LexiconConfigService,
  ...
  $onInit(): void {
    if (this.comment.regarding.field) {
      this.configService
        .getFieldConfig(this.comment.regarding.field)
        .then((config: LexConfigField) => {
          this.commentRegardingFieldConfig = config;
          this.isCommentRegardingPicture =
            this.commentRegardingFieldConfig.type === 'pictures' &&
            !this.comment.regarding.inputSystem;
        });
    }
  }
  ...
  showCommentReplies(): void {
    ...
    this.setCommentInteractiveStatus({
      id: this.comment.id,
      visible: this.comment.showRepliesContainer,
    });
    this.getSenseLabel();
  }
  ...
  canLike(): boolean {
    return (
      this.canPlusOneComment({ commentId: this.comment.id }) &&
    ...
    this.sessionService.getSession().then((session) => {
      if (session.userId() === comment.authorInfo.createdByUserRef.id) {
        ...
        deletemsg =
          ...
          comment.authorInfo.createdByUserRef.name +
          "'s comment?";
      }
  ...
  deleteCommentReply(commentId: string, reply: LexCommentReply): void {
    ...
    this.sessionService.getSession().then((session) => {
      if (session.userId() === reply.authorInfo.createdByUserRef.id) {
        ...
        deletemsg =
          'Are you sure you want to delete ' +
          reply.authorInfo.createdByUserRef.name +
          "'s comment reply?";
      }

      this.modal
        .showModalSimple('Delete Reply', deletemsg, 'Cancel', 'Delete Reply')
        .then(
          () => {
  ...
  getSenseLabel(): string {
    return this.parentGetSenseLabel({
      regardingField: this.comment.regarding.field,
      contextGuid: this.comment.contextGuid,
    });
  }
  ...
  getCurrentContextValue(): string {
    ...
    if (
      ...
      contextParts.fieldConfig.type === 'pictures' &&
  ...
  getCommentRegardingPictureSource(): string {
    ...
    if (contextParts.sense.guid) {
      pictures =
        this.control.currentEntry.senses[contextParts.sense.index].pictures;
    }

    for (const i in pictures) {
      if (
        pictures.hasOwnProperty(i) &&
        pictures[i].guid === contextParts.value
      ) {
...
export const CommentComponent: angular.IComponentOptions = {
  ...
  templateUrl:
    '/angular-app/languageforge/lexicon/editor/comment/dc-comment.component.html',
};
