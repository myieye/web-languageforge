...
export class LexiconEditorController implements angular.IController {
  ...
  getEntryCommentCount = this.commentService.getEntryCommentCount.bind(
    this.commentService
  );
  ...
  $onInit(): void {
    ...
    this.sendReceive.setSyncProjectStatusSuccessCallback(
      this.syncProjectStatusSuccess
    );

    this.$scope.$on('$locationChangeStart', (event, next, current) => {
      if (
        current.includes('#!/editor/entry') &&
        !next.includes('#!/editor/entry')
      ) {
    ...
    this.show.entryListModifiers = !(
      this.$window.localStorage.getItem('viewFilter') == null ||
      this.$window.localStorage.getItem('viewFilter') === 'false'
    );
  }

  $onChanges(changes: any): void {
    const finishedLoadingChange =
      changes.lecFinishedLoading as angular.IChangesObject<boolean>;
    ...
    const configChange =
      changes.lecConfig as angular.IChangesObject<LexiconConfig>;
    if (configChange != null && configChange.currentValue != null) {
      this.control.config = this.lecConfig;
    }

    const interfaceConfigChange =
      changes.lecInterfaceConfig as angular.IChangesObject<InterfaceConfig>;
    if (
    ...
    const projectChange =
      changes.lecProject as angular.IChangesObject<LexiconProject>;
  ...
  initFilterAndSearchOptions(): void {
    const clear = this.$scope.$watch(
      () => this.entryListModifiers.sortOptions.length > 0,
      (ready: boolean) => {
        if (!ready) return;
        clear(); // remove the watcher

        if (this.$state.params.sortBy) {
          const sortBy = this.findSelectedFilter(
            this.entryListModifiers.sortOptions,
            this.$state.params.sortBy
          );
          if (sortBy) {
            this.entryListModifiers.sortBy = sortBy;
          }
        }
        this.entryListModifiers.sortReverse =
          this.$state.params.sortReverse === 'true';
        this.entryListModifiers.wholeWord =
          this.$state.params.wholeWord === 'true';
        this.entryListModifiers.matchDiacritic =
          this.$state.params.matchDiacritic === 'true';
        ...
        if (this.$state.params.filterBy || this.$state.params.filterText) {
          this.entryListModifiers.filterBy = {
            ...
            option: this.$state.params.filterBy
              ? this.findSelectedFilter(
                  this.entryListModifiers.filterOptions,
                  this.$state.params.filterBy
                )
              : null,
          };
        }
  ...
  filterOptionsActive() {
    ...
    return (
      (mod.filterBy && mod.filterBy.option) ||
      mod.sortBy.value !== 'default' ||
      mod.sortReverse ||
      mod.wholeWord ||
      mod.matchDiacritic
    );
  ...
  shouldShowFilterReset() {
    ...
    return (
      modifiers.filterActive() ||
      modifiers.sortBy.value !== 'default' ||
      modifiers.sortReverse
    );
  }
  ...
  saveCurrentEntry = (
    ...
    if (this.hasUnsavedChanges() && this.lecRights.canEditEntry()) {
      ...
      this.currentEntry = LexiconEditorController.normalizeStrings(
        this.currentEntry
      );
      ...
      const entryForDiffing = this.removeCustomFieldsForDeltaUpdate(
        angular.copy(entryForUpdate)
      );
      const pristineEntryForDiffing = this.removeCustomFieldsForDeltaUpdate(
        this.prepEntryForUpdate(this.pristineEntry)
      );
      const diffForUpdate = isNewEntry
        ? undefined
        : {
            id: entryForUpdate.id,
            _update_deep_diff: diff(
              LexiconEditorController.normalizeStrings(pristineEntryForDiffing),
              entryForDiffing
            ),
          };
      ...
      return this.$q
        ...
        .then((data) => {
          ...
          if (!entry) {
            this.resetEntryLists(
              this.currentEntry.id,
              angular.copy(this.pristineEntry)
            );
          }
          ...
          this.editorService.refreshEditorData().then(() => {
            ...
            if (entry && isNewEntry) {
              this.setCurrentEntry(
                this.entries[
                  this.editorService.getIndexInList(entry.id, this.entries)
                ]
              );
  ...
  deleteEntry = (entry: LexEntry): void => {
    const deleteMsg =
      ...
      LexiconUtilityService.getLexeme(
        this.lecConfig,
        this.lecConfig.entry,
        entry
      ) +
      "'</b>";
    this.modal
      .showModalSimple('Delete Entry', deleteMsg, 'Cancel', 'Delete Entry')
      .then(
        () => {
          let iShowList = this.editorService.getIndexInList(
            entry.id,
            this.visibleEntries
          );
  ...
  makeValidModelRecursive = (
    config: LexConfigField,
    data: any = {},
    stopAtNodes: string | string[] = []
  ): any => {
    ...
    switch (config.type) {
      case 'fields':
        ...
        for (const fieldName of configFieldList.fieldOrder) {
          if (data[fieldName] == null) {
            if (
              configFieldList.fields[fieldName].type === 'fields' ||
          ...
          if (
            stopAtNodes.indexOf(fieldName) === -1 ||
            ...
            if (configFieldList.fields[fieldName].type === 'fields') {
              ...
              for (let i = 0; i < data[fieldName].length; i++) {
                data[fieldName][i] = this.makeValidModelRecursive(
                  configFieldList.fields[fieldName],
                  data[fieldName][i],
                  stopAtNodes
                );
              }
            } else {
              data[fieldName] = this.makeValidModelRecursive(
                configFieldList.fields[fieldName],
                data[fieldName],
                stopAtNodes
              );
      ...
      case 'multitext':
        ...
        for (const inputSystemTag of (config as LexConfigMultiText)
          .inputSystems) {
      ...
      case 'optionlist':
        if (data.value == null || data.value === null) {
          ...
          if (
            this.lecConfig.optionlists != null &&
            configOptionList.listCode != null &&
            configOptionList.listCode in this.lecConfig.optionlists &&
            this.lecConfig.optionlists[configOptionList.listCode]
              .defaultItemKey != null
          ) {
            data.value =
              this.lecConfig.optionlists[
                configOptionList.listCode
              ].defaultItemKey;
          }
        }
      ...
      case 'pictures':
        ...
        for (const picture of data as LexPicture[]) {
          ...
          picture.caption = this.makeValidModelRecursive(
            captionConfig,
            picture.caption
          );
        }
  ...
  getWordForDisplay(entry: LexEntry): string {
    const lexeme: string = LexiconUtilityService.getLexeme(
      this.lecConfig,
      this.lecConfig.entry,
      entry
    );
  ...
  getPrimaryListItemForDisplay(entry: LexEntry) {
    return (
      this.highlightMatches(this.getSortableValue(this.lecConfig, entry)) ||
      '[Empty]'
    );
  }
  ...
  getSecondaryListItemForDisplay(entry: LexEntry): string {
    return (
      this.highlightMatches(this.getMeaningForDisplay(this.lecConfig, entry)) ||
      '[Empty]'
    );
  }
  ...
  getFontFamilyForSecondaryListItemForDisplay(entry: LexEntry) {
    ...
    return this.lecConfig.inputSystems[
      (
        (this.lecConfig.entry.fields.senses as LexConfigFieldList).fields
          .gloss as LexConfigMultiText
      ).inputSystems[0]
    ].cssFontFamily;
  }
  ...
  highlightMatches(text: string) {
    ...
    while (upperCaseText.indexOf(filterText, previousIndex) !== -1) {
      ...
      output +=
        text.slice(previousIndex, resultIndex) +
        '<span class="highlight-result">' +
        text.slice(resultIndex, end) +
        '</span>';
      previousIndex = end;
    }
  ...
  showComments(): void {
    if (
      ...
      this.setCommentContext('');
      const commentsRightPanel = document.querySelector(
    ...
    if (this.rightPanelVisible === false) {
      ...
      if (currentElement.classList.contains('panel-visible')) {
        ...
        this.$interval(
          () => {
            document
              .querySelector('.panel-closing')
              .classList.remove('panel-closing', 'panel-switch');
          },
          500,
          1
        );
  ...
  getContextParts = (contextGuid: string) => {
    ...
    if (senseGuid) {
      for (const a in currentEntry.senses) {
        if (
          currentEntry.senses.hasOwnProperty(a) &&
          currentEntry.senses[a].guid === senseGuid
        ) {
          senseIndex = a;
          if (exampleGuid) {
            for (const b in currentEntry.senses[a].examples) {
              if (
                currentEntry.senses[a].examples.hasOwnProperty(b) &&
    ...
    if (exampleGuid && exampleIndex) {
      if (
        currentEntry.senses[senseIndex].examples[exampleIndex].hasOwnProperty(
          field
        )
      ) {
        currentField =
          currentEntry.senses[senseIndex].examples[exampleIndex][field];
    ...
    if (currentField !== null) {
      ...
      if (
        fieldConfig !== null &&
        (fieldConfig.type === 'multioptionlist' ||
          fieldConfig.type === 'optionlist')
      ) {
        if (field === 'semanticDomain') {
          ...
          outerFor: for (const listCode in optionLists) {
            if (
              ...
              listCode === (fieldConfig as LexConfigOptionList).listCode
            ) {
    ...
    this.editorService.loadEditorData().then(async () => {
      if (this.$state.is('editor.entry')) {
        if (this.entries.length > 0) {
          ...
          // if entry not found
          if (
            this.editorService.getIndexInList(entryId, this.entries) == null
          ) {
            entryId = '';

            // see if there is a most-recently viewed entry in the cache
            await this.offlineCacheUtils
              .getProjectMruEntryData()
              .then((data) => {
                if (
                  data &&
                  data.mruEntryId &&
  ...
  private findSelectedFilter<T extends LabeledOption>(
    collections: T[],
    params: string
  ): T {
    if (collections && params)
      return collections.filter((item) => item.label === params)[0];
  }
  ...
  private setSortAndFilterOptionsFromConfig(): void {
    ...
    for (const entryFieldName of this.lecConfig.entry.fieldOrder) {
      ...
      if (entryFieldName === 'senses') {
        const configSenses = this.lecConfig.entry.fields
        ...
        for (const senseFieldName of configSenses.fieldOrder) {
          ...
          if (senseField.type === 'multitext') {
            for (const inputSystemTag of (senseField as LexConfigMultiText)
              .inputSystems) {
              const abbreviation =
                this.getInputSystemAbbreviation(inputSystemTag);
              filterOptions.push({
                label: senseField.label + ' [' + abbreviation + ']',
                level: 'sense',
                value: senseFieldName,
                type: 'multitext',
                inputSystem: inputSystemTag,
                key: senseFieldName + '-' + inputSystemTag,
              });
            }
          } else {
            filterOptions.push({
              label: senseField.label,
              level: 'sense',
              value: senseFieldName,
              type: senseField.type,
              key: senseFieldName,
            });
          }
        }
        ...
        if (entryField.type === 'multitext') {
          for (const inputSystemTag of (entryField as LexConfigMultiText)
            .inputSystems) {
            const abbreviation =
              this.getInputSystemAbbreviation(inputSystemTag);
            filterOptions.push({
              ...
              level: 'entry',
              value: entryFieldName,
              type: 'multitext',
              inputSystem: inputSystemTag,
              key: entryFieldName + '-' + inputSystemTag,
            });
          }
        } else {
          filterOptions.push({
    ...
    for (const inputSystemsTag in this.lecConfig.inputSystems) {
      if (
        this.lecConfig.inputSystems.hasOwnProperty(inputSystemsTag) &&
        LexiconUtilityService.isAudio(inputSystemsTag)
      ) {
    ...
    LexiconUtilityService.arrayCopyRetainingReferences(
      sortOptions,
      this.entryListModifiers.sortOptions
    );
    LexiconUtilityService.arrayCopyRetainingReferences(
  ...
  private resetEntryLists(id: string, pristineEntry: LexEntry): void {
    ...
    const visibleEntryIndex = this.editorService.getIndexInList(
      id,
      this.visibleEntries
    );
  ...
  private getInputSystemAbbreviation(inputSystemTag: string): string {
    if (
      this.lecConfig == null ||
      this.lecConfig.inputSystems == null ||
  ...
  private setCurrentEntry(entry: LexEntry = new LexEntry()): void {
    ...
    entry = this.makeValidModelRecursive(
      this.lecConfig.entry,
      entry,
      'examples'
    );
  ...
  private prepEntryForUpdate(entry: LexEntry): LexEntry {
    const entryForUpdate: LexEntry = this.recursiveRemoveProperties(
      angular.copy(entry),
    ...
    if ('customFields' in data) {
      ...
      if ('senses' in data) {
        data.senses = data.senses.map((sense: any) =>
          this.removeCustomFieldsForDeltaUpdate(sense)
        );
      }
      if ('examples' in data) {
        data.examples = data.examples.map((example: any) =>
          this.removeCustomFieldsForDeltaUpdate(example)
        );
      }
    }
    return data;
  ...
  private warnOfUnsavedEdits(entry: LexEntry): void {
    this.warnOfUnsavedEditsId = this.notice.push(
      this.notice.WARN,
      'A synchronize has been started by ' +
  ...
  private pollUpdateSuccess = (): void => {
    if (this.hasUnsavedChanges()) {
      ...
      this.setCurrentEntry(
        this.entries[
          this.editorService.getIndexInList(this.currentEntry.id, this.entries)
        ]
      );
    }
  };
  ...
  private scrollListToEntry(id: string, alignment: string = 'center'): void {
    ...
    const interval = this.$interval(
      () => {
        if ($(entryDivId)[0]) {
          LexiconEditorController.syncListEntryWithCurrentEntry(
            entryDivId,
            alignment
          );
      ...
      200,
      30
    ); // 200ms delay, 30 tries max
  }

  private static syncListEntryWithCurrentEntry(
    elementId: string,
...
export const LexiconEditorComponent: angular.IComponentOptions = {
  ...
  templateUrl:
    '/angular-app/languageforge/lexicon/editor/editor.component.html',
};
