...
export class LexiconAppController implements angular.IController {
  ...
  $onInit(): void {
    ...
    this.$q
      .all([
        this.rightsService.getRights(),
        this.configService.getEditorConfig(),
      ])
      .then(([rights, editorConfig]) => {
        ...
        this.$scope.$watch(
          () => this.interfaceConfig.languageCode,
          (newVal: string) => {
            if (
              newVal &&
              this.pristineLanguageCode &&
              newVal !== this.pristineLanguageCode
            ) {
      ...
      .then(() => {
        // end of path "A" -- user cannot edit
        if (finishedPreloading && !this.finishedLoading) this.postLoad();
      });
  ...
  onUpdate = ($event: {
    ...
    if ($event.config || $event.optionLists) {
      this.configService
        .getEditorConfig(this.config, this.optionLists)
        .then((configEditor) => {
  ...
  private setupConfig(rights: Rights, editorConfig: LexiconConfig): void {
    ...
    this.config =
      rights.session.projectSettings<LexiconProjectSettings>().config;
    this.optionLists =
      rights.session.projectSettings<LexiconProjectSettings>().optionlists;
    this.interfaceConfig =
      rights.session.projectSettings<LexiconProjectSettings>().interfaceConfig;
  ...
  private setupOffline(): void {
    ...
    function setTitle(
      ...
      (
        document.querySelector(
          'nav .navbar-brand .website-title'
        ) as HTMLElement
      ).textContent = text;
      (
        document.querySelectorAll('nav.navbar')[0] as HTMLElement
      ).style.backgroundColor = backgroundColorA;
      (
        document.querySelectorAll('nav.navbar-expand')[1] as HTMLElement
      ).style.backgroundColor = backgroundColorB;
    }
    ...
    Offline.on('down', () => {
      ...
      offlineMessageId = this.notice.push(
        this.notice.ERROR,
        'You are offline. Some features are not available',
        null,
        true,
        5 * 1000
      );
