...
class EntryListModifiers {
  ...
  filterByLabel = () =>
    (this.filterBy && this.filterBy.option && this.filterBy.option.label) || '';
  filterActive = () =>
    !!(this.filterText() || (this.filterBy && this.filterBy.option));
  sortOptionLabel = (s: string) =>
    s === 'Default'
      ? `Default (${this.filterText() ? 'Relevance' : 'Word'})`
      : s;
}
...
export class EditorDataService {
  readonly browserInstanceId: string = Math.floor(
    Math.random() * 1000000
  ).toString();
  ...
  constructor(
    ...
    private readonly cache: EditorOfflineCacheService,
    ...
    if (this.visibleEntries.length < this.filteredEntries.length) {
      UtilityService.arrayCopyRetainingReferences(
        this.filteredEntries.slice(
          0,
          this.visibleEntries.length + entriesIncrement
        ),
        this.visibleEntries
      );
    }
  ...
  loadEditorData = (): angular.IPromise<any> => {
    ...
    if (this.entries.length === 0) {
      ...
      if (this.cache.canCache()) {
        ...
        this.loadDataFromOfflineCache().then(
          (projectObj: any) => {
            if (projectObj.isComplete) {
              this.showInitialEntries().then(() => {
                this.notice.cancelLoading();
                this.refreshEditorData(projectObj.timestamp).then(
                  (result: any) => {
              ...
              this.entries = [];
              console.log(
                'Editor: cached data was found to be incomplete. Full download started...'
              );
          ...
          () => {
            ...
            console.log(
              'Editor: no data found in cache. Full download started...'
            );
            this.notice.setLoading('Downloading Full Dictionary.');
            this.notice.setPercentComplete(0);
            this.doFullRefresh().then((result) => {
  ...
  removeEntryFromLists = (id: string): angular.IPromise<any> => {
    angular.forEach(
      [this.entries, this.filteredEntries, this.visibleEntries],
      (list) => {
  ...
  processEditorDto(
    ...
    if (result.ok) {
      this.commentService.comments.counts.userPlusOne =
        result.data.commentsUserPlusOne;
      if (!updateOnly) {
        UtilityService.arrayExtend(this.entries, result.data.entries);
        this.commentService.comments.items.all.push.apply(
          this.commentService.comments.items.all,
        ...
        angular.forEach(
          result.data.deletedCommentIds,
          this.commentService.removeCommentFromLists
        );

        // only sort and filter the list if there have been changes to entries (or deleted entries)
        if (
          result.data.entries.length > 0 ||
          result.data.deletedEntryIds.length > 0
        ) {
  ...
  sortEntries = (
    shouldResetVisibleEntriesList: boolean
  ): angular.IPromise<any> => {
    ...
    return this.configService.getEditorConfig().then((config) => {
      ...
      UtilityService.arrayCopyRetainingReferences(
        filteredEntriesSorted,
        this.filteredEntries
      );
      if (shouldResetVisibleEntriesList) {
        UtilityService.arrayCopyRetainingReferences(
          filteredEntriesSorted.slice(0, entriesIncrement),
          this.visibleEntries
        );
      }
  ...
  filterEntries = (
    ...
    return this.configService.getEditorConfig().then((config) => {
      ...
      if (shouldResetVisibleEntriesList) {
        UtilityService.arrayCopyRetainingReferences(
          this.filteredEntries.slice(0, entriesIncrement),
          this.visibleEntries
        );
      }
    });
  ...
  getMeaningForDisplay(config: LexiconConfig, entry: LexEntry): string {
    ...
    if (entry.senses && entry.senses[0]) {
      meaning = LexiconUtilityService.getMeaning(
        config,
        config.entry.fields.senses as LexConfigFieldList,
        entry.senses[0]
      );
    }
  ...
  getSortableValue = (config: any, entry: any): string => {
    const fieldKey =
      this.entryListModifiers.sortBy.value === 'default'
        ? 'lexeme'
        : this.entryListModifiers.sortBy.value;
    let sortableValue = '';
    let field;
    let dataNode;
    const isSpecialMultitext =
      fieldKey === 'lexeme' || fieldKey === 'citationForm';
    if (fieldKey in config.entry.fields && fieldKey in entry) {
      field = config.entry.fields[fieldKey];
      dataNode = entry[fieldKey];
    } else if (
      fieldKey in config.entry.fields.senses.fields &&
      angular.isDefined(entry.senses) &&
      entry.senses.length > 0 &&
      fieldKey in entry.senses[0]
    ) {
      ...
      if (isSpecialMultitext || field.type === 'multitext') {
        ...
        if (fieldKey === 'lexeme' || fieldKey === 'citationForm') {
          if (config.entry.fields.citationForm) {
            const citationFormInputSystems =
              config.entry.fields.citationForm.inputSystems;
            if (
              entry.citationForm &&
              citationFormInputSystems.length > 0 &&
              citationFormInputSystems[0] in entry.citationForm
            ) {
              sortableValue =
                entry.citationForm[citationFormInputSystems[0]].value;
            }
          }
          ...
          if (!sortableValue) {
            ...
            if (
              entry.lexeme &&
              lexemeInputSystems.length > 0 &&
              lexemeInputSystems[0] in entry.lexeme
            ) {
          ...
          if (
            field.inputSystems.length > 0 &&
            field.inputSystems[0] in dataNode
          ) {
        ...
        if (config.optionlists && config.optionlists[field.listCode]) {
          ...
          sortableValue = this.getOptionListItem(
        ...
        field.type === 'multioptionlist' &&
        dataNode.values.length > 0
      ) {
    ...
    this.api.dbeDtoFull(
      ...
      (result: JsonRpcResult) => {
        ...
        this.notice.setPercentComplete(
          Math.round((newOffset * 100) / totalCount)
        );
  ...
  private walkEntry(
    ...
    node: any,
    cb: (value: string, isSemanticDomain: boolean) => void
  ) {
    for (const fieldName of config.fieldOrder) {
      ...
      if (childConfig.type === 'fields') {
        for (const element of childNode)
          this.walkEntry(childConfig as LexConfigFieldList, element, cb);
      } else if (childConfig.type === 'multitext') {
        for (const inputSystem of (childConfig as LexConfigMultiText)
          .inputSystems) {
          if (childNode[inputSystem] && childNode[inputSystem].value)
            cb(childNode[inputSystem].value, false);
        }
      } else if (
        childConfig.type === 'multioptionlist' &&
        fieldName === 'semanticDomain'
      ) {
        for (const element of (childNode as LexMultiValue).values)
          cb(element, true);
      }
    }
  }
  ...
  private semanticDomainsMatch(text: string, query: string): boolean {
    ...
    return (
      ...
      (text.indexOf(query) === 0 &&
        !UtilityService.isDigitsOnly(
          text.slice(query.length - 1, query.length + 1)
        ))
    );
  }
  ...
  private entryMeetsFilterCriteria(config: any, entry: LexEntry): boolean {
    if (this.entryListModifiers.filterText() !== '') {
      ...
      const normalizedQuery = this.entryListModifiers.matchDiacritic
        ? rawQuery
        : this.removeDiacritics(rawQuery);
      ...
      this.walkEntry(config.entry, entry, (val, isSemanticDomain) => {
        if (isSemanticDomain && this.semanticDomainsMatch(val, rawQuery)) {
          ...
          const normalizedValue = this.entryListModifiers.matchDiacritic
            ? val
            : this.removeDiacritics(val);
    ...
    if (
      ['comments', 'exampleSentences', 'pictures', 'audio'].indexOf(
        filterType
      ) !== -1
    ) {
      ...
      switch (filterType) {
        ...
        case 'audio':
          ...
          angular.forEach(
            config.entry.fields,
            (entryField, entryFieldKey: string) => {
              ...
              if (entryFieldKey === 'senses') {
                angular.forEach(entry.senses, (sense) => {
                  angular.forEach(
                    config.entry.fields.senses.fields,
                    (senseField: any, senseFieldKey: string) => {
                      if (senseField.type === 'multitext') {
                        angular.forEach(
                          sense[senseFieldKey],
                          (fieldNode: any, ws: string) => {
                            if (
                              ws &&
                              UtilityService.isAudio(ws) &&
                              fieldNode.value !== ''
                      ...
                      if (senseFieldKey === 'examples') {
                        angular.forEach(sense.examples, (example) => {
                          angular.forEach(
                            config.entry.fields.senses.fields.examples.fields,
                            (exampleField: any, exampleFieldKey: string) => {
                              if (exampleField.type === 'multitext') {
                                angular.forEach(
                                  example[exampleFieldKey],
                                  (fieldNode: any, ws: string) => {
                                    if (
                                      ws &&
                                      UtilityService.isAudio(ws) &&
      ...
      if (this.entryListModifiers.filterBy.option.level === 'entry') {
        ...
        if (entry.senses && entry.senses.length > 0) {
          dataNode =
            entry.senses[0][this.entryListModifiers.filterBy.option.value];
        }
      }
        ...
        switch (filterType) {
          case 'multitext':
            if (dataNode[this.entryListModifiers.filterBy.option.inputSystem]) {
              containsData =
                dataNode[this.entryListModifiers.filterBy.option.inputSystem]
                  .value !== '';
            }
            break;
          case 'optionlist':
    ...
    return (
      (mustNotBeEmpty && containsData) || (!mustNotBeEmpty && !containsData)
    );
  }
  ...
  private loadDataFromOfflineCache(): angular.IPromise<any> {
    ...
    this.cache.getAllEntries().then(
      (entries) => {
        UtilityService.arrayExtend(this.entries, entries);
        numOfEntries = entries.length;
        if (entries.length > 0) {
          this.commentsCache.getAllComments().then(
            (comments) => {
              UtilityService.arrayExtend(
                this.commentService.comments.items.all,
                comments
              );
              this.cache.getProjectData().then(
                (projectData) => {
                  this.commentService.comments.counts.userPlusOne =
                    projectData.commentsUserPlusOne;
                  endTime = performance.now();
                  console.log(
                    'Editor: Loaded ' +
                      numOfEntries +
                      ' entries from cache in ' +
  ...
  filterAndSortEntries = (
    ...
    if (shouldResetVisibleEntriesList !== false)
      shouldResetVisibleEntriesList = true;
    ...
    if (
      ...
      for (const entry of list) {
        ...
        this.walkEntry(config.entry, entry, (val, isSemanticDomain) => {
          ...
          if (
            index === 0 &&
            (!isSemanticDomain || this.semanticDomainsMatch(val, queryCapital))
          ) {
      ...
      prioritizedResults = prioritizedResults.map((section) => {
        ...
        for (const entry of section) {
          const indexA = this.getSortableValue(config, entry)
            .toUpperCase()
            .indexOf(queryCapital);
          if (indexA !== -1) {
            subList[indexA === 0 ? 0 : 1].push(entry);
            continue;
          }
          const indexB = this.getMeaningForDisplay(config, entry)
            .toUpperCase()
            .indexOf(queryCapital);
          if (indexB !== -1) {
        ...
        return subList
          .map((subSubList) =>
            this.sortListAlphabetically(config, subSubList, false)
          )
          .reduce((main, sub) => main.concat(sub), []);
      });

      const flattenedResults = prioritizedResults.reduce(
        (main, sub) => main.concat(sub),
        []
      );
  ...
  private sortListAlphabetically(
    ...
    const compare =
      'Intl' in window
        ? Intl.Collator(inputSystem).compare
   ...
   */
  private storeDataInOfflineCache(
    data: any,
    isComplete: boolean
  ): angular.IPromise<any> {
    const deferred = this.$q.defer();
  ...
  private printLexemesInList(entryList: LexEntry[]): void {
    this.sessionService.getSession().then((session) => {
      ...
      const ws = (config.entry.fields.lexeme as LexConfigMultiText)
        .inputSystems[1];
      const arr = [];
