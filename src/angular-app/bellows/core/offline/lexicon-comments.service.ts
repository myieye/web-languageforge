...
import {
  LexComment,
  LexCommentReply,
} from '../../../languageforge/lexicon/shared/model/lex-comment.model';
...
export class LexiconCommentService {
  ...
  constructor(
    ...
    private offlineCache: CommentsOfflineCacheService,
  ...
  loadEntryComments(entryId: string): Promise<any> {
    ...
    for (const comment of this.comments.items.all) {
      if (comment.entryRef === entryId) {
        promises.push(
          this.lexConfig
            .getFieldConfig(comment.regarding.field)
            .then((fieldConfig) => {
              ...
              if (this.comments.items.currentEntry.indexOf(comment) === -1) {
                let contextGuid = '';
                if (
                  comment.contextGuid !== undefined &&
                  comment.contextGuid !== ''
                ) {
                  contextGuid = comment.contextGuid;
                } else {
                  contextGuid =
                    comment.regarding.field +
                    (comment.regarding.inputSystemAbbreviation
                      ? '.' + comment.regarding.inputSystemAbbreviation
                      : '');
                  if (
                    fieldConfig != null &&
                ...
                if (
                  contextGuid &&
                  this.comments.counts.currentEntry.fields[contextGuid] == null
                ) {
  ...
  removeCommentFromLists = (commentId: string, replyId?: string): void => {
    if (replyId) {
      ...
      LexiconCommentService.deleteCommentInList(
        commentId,
        replyId,
        this.comments.items.all
      );
      LexiconCommentService.deleteCommentInList(
        commentId,
        replyId,
        this.comments.items.currentEntry
      );
      LexiconCommentService.deleteCommentInList(
        commentId,
        replyId,
        this.comments.items.currentEntryFiltered
      );
    } else {
  ...
  refreshFilteredComments(filter: any): void {
    ...
    let comments = this.$filter('filter')(
      this.comments.items.currentEntry,
      filter.byText
    );
  ...
  updateStatus(
    ...
    return this.api.call(
      'lex_comment_updateStatus',
      [commentId, status],
      callback
    );
  }

  private static deleteCommentInList(
    commentId: string,
    replyId: string,
    commentsList: LexComment[]
  ): void {
