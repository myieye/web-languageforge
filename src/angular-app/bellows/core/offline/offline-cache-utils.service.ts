...
export class OfflineCacheUtilsService {
  ...
  private readonly interfaceStore = localforage.createInstance({
    name: 'interface',
  });

  static $inject: string[] = ['$q', 'sessionService', 'offlineCache'];
  constructor(
  ...
  getProjectData(): Promise<any> {
    return this.offlineCache.getOneFromStore(
      'projects',
  ...
  getInterfaceLanguageCode(): angular.IPromise<any> {
    return this.$q.when(
      this.interfaceStore.getItem<string>(this.INTERFACE_KEY_LANGUAGE_CODE)
    );
  }

  updateInterfaceLanguageCode(languageCode: string): angular.IPromise<any> {
    return this.$q.when(
      this.interfaceStore.setItem<string>(
        this.INTERFACE_KEY_LANGUAGE_CODE,
        languageCode
      )
    );
  }
