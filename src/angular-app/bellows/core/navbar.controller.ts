...
import {
  ...
  HeaderData,
} from './application-header.service';
...
export class NavbarController implements angular.IController {
    ...
    this.sessionService.getSession().then((session) => {
      ...
      if (this.project) {
        ...
        this.displayShareButton =
          this.currentUserIsProjectManager ||
          (this.project.allowSharing && this.session.data.userIsProjectMember);
      }

      this.rights.canCreateProject = session.hasSiteRight(
  ...
  private useLocallyStoredLanguageCode(): void {
    this.offlineCacheUtils
      .getInterfaceLanguageCode()
      .then((localLanguageCode) => {
