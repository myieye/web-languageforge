...
export class SessionService {
  ...
  ): angular.IPromise<SessionData> {
    if (this.sessionDataPromise && !forceRefresh)
      return this.sessionDataPromise;

    const promise: angular.IPromise<SessionData> = this.api
      .call('session_getSessionData')
      .then((result) => {
        return result.data;
      })
      .catch((result) => {
